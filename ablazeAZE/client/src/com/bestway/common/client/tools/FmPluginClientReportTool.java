...
public class FmPluginClientReportTool extends JInternalFrameBase {
	 ...
	 */
	public FmPluginClientReportTool() {
		super();
	...
	private void deletePlugin() {
		PluginInfoClientReport pluginInfoClient = (PluginInfoClientReport) tableModel
				.getCurrentRow();
		if (pluginInfoClient == null) {
			JOptionPane.showMessageDialog(FmPluginClientReportTool.this,
					"请选择要删除的插件：！", "提示", 2);
			return;
		}
		...
		boolean isSuccess = toolsAction.deleteReportPlugin(new Request(
	...
	private File getJarFile() {
		...
		int state = fileChooser.showDialog(FmPluginClientReportTool.this, "选择插件文件");
		...
		public void run() {
			...
			try {
				...
				byte[] bytes = CommonUtils.getBytesByFile(file
						.getAbsolutePath());
				toolsAction.uploadReportPlugin(new Request(CommonVars.getCurrUser()),
						file.getName(), bytes);
				//
				// 刷新插件
				//
				...
				JOptionPane.showMessageDialog(FmPluginClientReportTool.this, info,
						"提示", 2);
			} catch (Exception e) {
				CommonProgress.closeProgressDialog(flag);
				JOptionPane.showMessageDialog(FmPluginClientReportTool.this,
	...
	private String getSaveHsqlFileName() {
		...
		int state = fileChooser.showDialog(FmPluginClientReportTool.this, "保存插件文件");
		if (state == JFileChooser.APPROVE_OPTION) {
	...
	class DownloadPluginThread extends Thread {
		...
		PluginInfoClientReport	pluginInfoClient	= null;

		public DownloadPluginThread(PluginInfoClientReport pluginInfoClient) {
		...
		public void run() {
			...
			try {
				...
				byte[] bytes = toolsAction.downloadReportPlugin(new Request(
						CommonVars.getCurrUser()), pluginInfoClient);
				CommonProgress.closeProgressDialog(flag);
		...
		tableModel = new JTableListModel(this.tbPlugin, list,
				new JTableListModelAdapter() {
					public List<JTableListColumn> InitColumns() {
						...
						list.add(addColumn("最后修改时间", "modifiyTime", 255));
						return list;
					}
				});
	}
	...
	private JButton getBtnDeleteAll() {
		if (btnDeleteAll == null) {
			...
			btnDeleteAll.setText("删除所有客户端报表插件");
	...
	class DeleteAllPluginThread extends Thread {
		public void run() {
			...
			try {
				...
				toolsAction.deleteAllReportPlugin(new Request(CommonVars
						.getCurrUser()));
				CommonProgress.closeProgressDialog(flag);
