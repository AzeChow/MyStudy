 ...
 */
public class DgTxtTaskExDetail extends JDialogBase {
		...
		this.addWindowListener(new java.awt.event.WindowAdapter() {
			public void windowOpened(java.awt.event.WindowEvent e) {
				...
				if (tableModel.getCurrentRow() != null) {
					...
					List list = dataImportAction.findTxtTaskSel(new Request(
					...
					DgTxtTaskExDetail.this.jLabel.setText(DgTxtTaskExDetail.this.getTitleName());
			    }
			}
		});
	}
		...
		if (jButton == null) {
			...
			jButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					DgTxtTaskExDetail.this.dispose();
				}
			});
		}
		return jButton;
		...
		tableModel = new JTableListModel(jTable, dataSource,
				new JTableListModelAdapter() {
					public List InitColumns() {
						...
						list.add(addColumn("导入格式名称", "txttable.taskname", 150));
						list.add(addColumn("目标表", "txttable.classList.name", 100));
						list.add(addColumn("字符集转换", "txttable.gbflag", 120));
						list.add(addColumn("格式创建者", "txttable.creator", 120));
						return list;
					}
				});
