 ...
 */
public class DgCopyHeadAndItems extends JDialogBase {
	...
	private PnCopyItems pnCopyItems = null;
	...
	private void done() {
		...
		List<FptAppItem> contractExgList = this.getPnCopyItems()
		...
		fptAppHead = this.fptManageAction.copyFptAppItem(new Request(
				CommonVars.getCurrUser(),true), fptAppHead, contractExgList);
		isOk = true;
		this.dispose();
	}
	...
	private PnCopyItems getPnCopyItems() {
		if (pnCopyItems == null) {
			pnCopyItems = new PnCopyItems();
			pnCopyItems.setBounds(0, 0, 500, 248);
		}
		return pnCopyItems;
	}
	...
	private void gotoStep(int step) {
		...
		case 0:
			...
			this.pnContext.add(this.getPnCopyItems(), null);
			this.getPnCopyItems().setFptAppHead(fptAppHead);
			this.getPnCopyItems().setVisible(true);			
			this.lbHeadText.setText("第一步：选择转抄的明细");
			break;		
		}
	...
	private void setState() {
		...
		this.btnNext.setEnabled(this.step < 0);
		this.btnDone.setEnabled(this.step == 0);
	}
