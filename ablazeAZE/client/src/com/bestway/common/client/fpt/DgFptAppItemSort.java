 ...
 */
public class DgFptAppItemSort extends JDialogBase {
	...
	public void setVisible(boolean b) {
		if (b) {
			...
			for (int i = 0; i < list.size(); i++) {
				FptAppItem obj = (FptAppItem) list.get(i);
	...
	class UserListCellRenderer extends DefaultListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			String s = "";
			if (value != null) {
				FptAppItem obj = ((FptAppItem) value);
				s = formatStringByLength(obj.getListNo().toString(), 5)
						+ formatStringByLength(obj.getCodeTs() == null ? ""
								: obj.getCodeTs().getCode(), 20)
	...
	private void move(boolean isup) {
		if (jList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(DgFptAppItemSort.this, "请选择要移动的项!",
					"提示", 2);
			return;
		}
		...
		FptAppItem selectinfo = null;
		FptAppItem preinfo = null;
		if (isup) {// 向上移动
			selectinfo = (FptAppItem) selectObj;
			selectinfo.setListNo(selectinfo.getListNo() - 1);
			preinfo = (FptAppItem) jList.getModel().getElementAt(x - 1);
	...
	private JButton getBtnOk() {
		if (btnOk == null) {
			...
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Vector list = DgFptAppItemSort.this.getList();
					fptManageAction.saveFptAppItem(new Request(CommonVars
							.getCurrUser()), list);
					isOk = true;
					dispose();
				}
