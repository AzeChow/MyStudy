 ...
 */
public class DgFptBillItemSort extends JDialogBase {
	...
	public void setVisible(boolean b) {
		if (b) {
			...
			for (int i = 0; i < list.size(); i++) {
				FptBillItem obj = (FptBillItem) list.get(i);
				obj.setListNo(i + beginSeqNum);
				listS.add(obj);
			}
	...
	class UserListCellRenderer extends DefaultListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			String s = "";
			if (value != null) {
				FptBillItem obj = ((FptBillItem) value);
				s = formatStringByLength(obj.getListNo().toString(), 5)
	...
	private void move(boolean isup) {
		if (jList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(DgFptBillItemSort.this, "请选择要移动的项!",
					"提示", 2);
			return;
		}
		...
		FptBillItem selectinfo = null;
		FptBillItem preinfo = null;
		if (isup) {// 向上移动
			selectinfo = (FptBillItem) selectObj;
			selectinfo.setListNo(selectinfo.getListNo() - 1);
			preinfo = (FptBillItem) jList.getModel().getElementAt(x - 1);
			preinfo.setListNo(preinfo.getListNo() + 1);
	...
	private JButton getBtnOk() {
		if (btnOk == null) {
			...
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Vector list = DgFptBillItemSort.this.getList();
					fptManageAction.saveFptBillItem(new Request(CommonVars
							.getCurrUser()), list);
					isOk = true;
					dispose();
				}
	 ...
	 */
	public void setFptManageAction(FptManageAction baseEncAction) {
		this.fptManageAction = baseEncAction;
	}
