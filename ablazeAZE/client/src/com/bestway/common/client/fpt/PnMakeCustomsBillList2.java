...
public class PnMakeCustomsBillList2 extends PnCommon {
	...
	private void selectAll(boolean isSelected) {
		...
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i) instanceof TempFptBillHead) {
				TempFptBillHead t = (TempFptBillHead) list
						.get(i);
...
//				new MultiRenderer());
	...
	class CheckBoxEditor extends DefaultCellEditor implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (obj instanceof TempFptBillHead) {
				TempFptBillHead temp = (TempFptBillHead) obj;
	...
	public boolean checkSelectedList() {
		...
		String code = "";
		if (list.size() > 0) {
			TempFptBillHead t = (TempFptBillHead) list.get(0);
			code = t.getT().getAppNo();
		}
		for (int i = 0; i < list.size(); i++) {
			TempFptBillHead t = (TempFptBillHead) list.get(i);
			if (!t.getT().getAppNo().equals(code)) {
				return false;
			}
		}
		return true;
