...
import com.bestway.common.client.fpt.DgImportDataExg.ImportFileData;
import com.bestway.common.client.fpt.DgImportDataExg.ImportFileDataRunnable;
...
public class DgImportDataExg extends JDialogBase {
	...
	private List parseTxtFile(File file) {
		...
		System.out.println("循环保存和判断成品数据开始");
	...
	private void checkDate(List lines,List<String> listAppHead,List<String> listAppItem){
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			line.add(mapIndex.get("错误信息"), errorInformation);
	...
	public void saveData(List<String> line,Hashtable<String, FptBillHead> mapHead,
		...
		if(head==null){
			...
			head.setIssueAgentCode(fptAppHead.getOutAgentCode());
			head.setIssueAgentName(fptAppHead.getOutAgentName());
			...
			head.setIssueNote(line.get(mapIndex.get("表头备注")));
			head.setIssueIsDeclaEm(line.get(mapIndex.get("申报人(必填)")));
	...
	public Hashtable<Integer, Boolean> getSerialNumber(){
		...
		//查找流水号
		List lsMumber = fptManageAction.findFptBillHeadSerialNumber(request,FptInOutFlag.OUT);;
	...
	public Hashtable<String,Hashtable<Integer,FptAppItem>> getFptAppItem(List list){
		...
		List<FptAppItem> dataSource = fptManageAction.findFptAppItemToFptBillItem(new Request(CommonVars.getCurrUser()), FptInOutFlag.OUT, list);
		for (int i = 0; i < dataSource.size(); i++) {
			...
			Hashtable<Integer,FptAppItem> mapItem = mapApp.get(appNo);
			if(mapItem==null){
				mapItem = new Hashtable<Integer,FptAppItem>();
	...
	public Hashtable<String,FptAppHead> getFptAppHead(List list){
		...
		List<FptAppHead> dataSource = fptManageAction.findFptAppHead(new Request(CommonVars.getCurrUser()), FptInOutFlag.OUT,list);
		for (int i = 0; i < dataSource.size(); i++) {
			...
			btnOpenFile.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					fileChooser.removeChoosableFileFilter(fileChooser
							.getFileFilter());
					fileChooser.setSelectedFile(new File("C:\\Users\\Administrator\\Desktop\\成品发货单导入样板.xls"));
	...
	class ImportFileData extends Thread {
		...
		public void run() {
				...
				try {
					...
					//保存导入的数据至数据库中
					fptManageAction.saveFptBillHeadsAndFptBillItems(request,listImg,FptBusinessType.FPT_BILL);
					CommonProgress.closeProgressDialog();
					JOptionPane.showMessageDialog(DgImportDataExg.this,
					...
					CommonProgress.closeProgressDialog();
					JOptionPane.showMessageDialog(DgImportDataExg.this, e2.getMessage());
				}finally{
					CommonProgress.closeProgressDialog();
				}
