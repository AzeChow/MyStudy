 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class DgPutRecordPlan extends JDialogBase {
	...
	private void initialize() {
		...
		transferFactoryManageAction.findPutRecord(new Request(
		...
		tableModel = new JTableListModel(jTable, list,
				new JTableListModelAdapter() {
					...
					public List InitColumns() {
						...
						list.add(addColumn("现有备案数", "billSum1", 80));
						list.add(addColumn("上月累计结余", "billSum2", 80));
						list.add(addColumn("验收数量", "billSum7", 60));
						list.add(addColumn("验收重量", "billSum8", 60));				
						list.add(addColumn("转厂", "billSum3", 50));
						list.add(addColumn("收货未结转", "billSum4", 70));	
						list.add(addColumn("验收(未来21天数量)", "billSum6", 120));
						list.add(addColumn("本周备案差异(备案-送货)", "billSum5", 150));
						list.add(addColumn("未来备案差异(备案-送货)", "billSum6", 150));
						list.add(addColumn("下周应安排备案32笔", "billSum6", 120));
						list.add(addColumn("下周实际安排备案", "billSum6", 100));
						list.add(addColumn("备注", "billSum6", 60));
						return list;
					}
				});
	...
	class find extends Thread{
		...
		public void run(){
			 try {
				...
				lsResult=transferFactoryManageAction.findPutRecord(new Request(
						CommonVars.getCurrUser()));
				
				CommonProgress.closeProgressDialog();
			 } catch (Exception e) {
		        CommonProgress.closeProgressDialog();
		        JOptionPane.showMessageDialog(DgPutRecordPlan.this,
