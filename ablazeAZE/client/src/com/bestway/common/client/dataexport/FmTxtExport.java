...
public class FmTxtExport extends JInternalFrameBase {
	...
	private JRadioButton getRbRegion() {
		if (rbRegion == null) {
			...
			rbRegion.setText("文本域定义管理");
			rbRegion.setBounds(312, 245, 119, 21);
	...
	private JRadioButton getRbTaskPlan() {
		if (rbTaskPlan == null) {
			...
			rbTaskPlan.setText("文本任务计划设置");
			rbTaskPlan.setBounds(336, 279, 130, 21);
	...
	private JPanel getJPanel2() {
		if (jPanel2 == null) {
			...
			jLabel1.setBounds(2, 22, 518, 356);
			jLabel1.setText("");
			jLabel1
					.setIcon(new ImageIcon(
							getClass()
	...
	private JRadioButton getRbExecute() {
		if (rbExecute == null) {
			...
			rbExecute.setText("执行文本导入导出");
			rbExecute.setBounds(353, 309, 154, 21);
	...
	private class RadioActionListner implements ActionListener {
		public void actionPerformed(java.awt.event.ActionEvent e) {
			if (rbViewPara.isSelected()) { //SQL 参数设置
				String flag = rbViewPara.getActionCommand();
                JDialogBase dialog = JDialogBaseHelper
                        .getJDialogBaseByFlag(flag);
                if (dialog == null) {
                	dataExportAction.checkSqlParameterSetExportAuthority(new Request(CommonVars
            				.getCurrUser()));
                ...
                JDialogBase dialog = JDialogBaseHelper
                        .getJDialogBaseByFlag(flag);
                if (dialog == null) {
                	dataExportAction.checkDataSourceOperateExportAuthority(new Request(CommonVars
            				.getCurrUser()));
				...
				dataExportAction.checkEventSetExportAuthority(new Request(CommonVars
        				.getCurrUser()));
                ...
                JDialogBase dialog = JDialogBaseHelper
                        .getJDialogBaseByFlag(flag);
                if (dialog == null) {
                	dataExportAction.checkViewOperateExportAuthority(new Request(CommonVars
            				.getCurrUser()));
			...
			} else if (rbRegion.isSelected()) { //文本 域定义管理
				
				String flag = rbRegion.getActionCommand();				
                JDialogBase dialog = JDialogBaseHelper
                        .getJDialogBaseByFlag(flag);
                if (dialog == null) {
                	dataExportAction.checkTextFieldExportAuthority(new Request(CommonVars
            				.getCurrUser()));
                	DgTxtRegion dg = new DgTxtRegion();
                    JDialogBaseHelper.putJDialogBaseToFlag(flag,
                    		dg);
                    dg.setVisible(true);
                } else {
			...
			} else if (rbTaskPlan.isSelected()) { // 文本任务计划设置
				
				String flag = rbTaskPlan.getActionCommand();				
                JDialogBase dialog = JDialogBaseHelper
                        .getJDialogBaseByFlag(flag);
                if (dialog == null) {
                	dataExportAction.checkTextRegionExportAuthority(new Request(CommonVars
            				.getCurrUser()));
                	DgTxtTask dg = new DgTxtTask();
                    JDialogBaseHelper.putJDialogBaseToFlag(flag,
                    		dg);
                    dg.setVisible(true);
                } else {
			...
			} else if (rbExecute.isSelected()) { // 执行 TXT DB 导出
				
				String flag = rbExecute.getActionCommand();				
                JDialogBase dialog = JDialogBaseHelper
                        .getJDialogBaseByFlag(flag);
                if (dialog == null) {
                	dataExportAction.checkTextTaskExeExportAuthority(new Request(CommonVars
            				.getCurrUser()));
                	DgTxtTaskExe dg = new DgTxtTaskExe();
                    JDialogBaseHelper.putJDialogBaseToFlag(flag,
                    		dg);
                    dg.setVisible(true);
                } else {
