...
public class DgBcsDictPorImg extends JDialogBase {
	...
	private JLabel jLabel1151 = null;

	private Complex complex = null;

	private JButton btnComplex = null;

	private BcsDictPorHead bcsDictPorHead = null; // @jve:decl-index=0:
	...
	private void initialize() {
		this.setSize(new Dimension(526, 294));
		this.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		this.setTitle("备案资料库料件");
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			jLabel2 = new JLabel();
			jLabel2.setBounds(new Rectangle(29, 212, 55, 27));
			jLabel2.setText("归并序号");
			jLabel1151 = new JLabel();
			jLabel1151.setBounds(new Rectangle(244, 177, 78, 24));
			jLabel1151.setText("主料/辅料");
			jLabel117 = new JLabel();
			jLabel117.setBounds(new Rectangle(245, 209, 29, 22));
			...
			jContentPane.add(jLabel1151, null);
	...
	private JTextField getTfComplex() {
		if (tfComplex == null) {
			...
			tfComplex.setBounds(new Rectangle(321, 50, 145, 24));
	...
	private JButton getBtnSave() {
		if (btnSave == null) {
			...
			btnSave.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					BcsDictPorImg img = (BcsDictPorImg) tableModel
							.getCurrentRow();
					if (img == null) {
						return;
					}
					fillData(img);
					img = bcsDictPorAction.saveBcsDictPorImg(new Request(
	...
	private JTextField getTfMemo() {
		if (tfMemo == null) {
			...
			tfMemo.setBounds(new Rectangle(274, 209, 214, 24));
	...
	private void setState() {
		BcsDictPorImg img = (BcsDictPorImg) tableModel.getCurrentRow();
		if (img == null) {
			return;
		}
		...
		this.cbIsMainImg.setEnabled(dataState != DataState.BROWSE);
	...
	private void showData() {
		...
		this.tfComplex.setText(img.getComplex().getCode());
		this.tfCommName.setText(img.getCommName());
		this.tfCommSpec.setText(img.getCommSpec());
		this.cbbUnit.setSelectedItem(img.getComUnit());
		this.tfFirstUnit.setText(img.getComplex().getFirstUnit().getName());
		this.cbbCurr.setSelectedItem(img.getCurr());
		this.tfUnitPrice.setValue(img.getUnitPrice());
		this.tfMemo.setText(img.getMemo());
		this.cbIsMainImg.setSelected(img.getIsMainImg() == null ? false : img
				.getIsMainImg());
		this.tfModifyMark.setText(ModifyMarkState.getModifyMarkSpec(img
				.getModifyMark()));
		this.tfInnerMergeSeqNum.setValue(img.getInnerMergeSeqNum());
	}

	private void fillData(BcsDictPorImg img) {
		if (img == null) {
			return;
		}
		img.setComplex(this.complex);
		img.setCommName(this.tfCommName.getText().trim());
		img.setCommSpec(this.tfCommSpec.getText().trim());
		img.setComUnit((Unit) this.cbbUnit.getSelectedItem());
		img.setCurr((Curr) this.cbbCurr.getSelectedItem());
		img.setUnitPrice(this.tfUnitPrice.getValue() == null ? 0.0 : Double
				.valueOf(this.tfUnitPrice.getValue().toString()));
		img.setMemo(this.tfMemo.getText().trim());
		img.setIsMainImg(this.cbIsMainImg.isSelected());
		if (this.tfInnerMergeSeqNum.getValue() != null) {
			img.setInnerMergeSeqNum(Double.valueOf(
					this.tfInnerMergeSeqNum.getValue().toString()).intValue());
		} else {
			img.setInnerMergeSeqNum(null);
		}
	...
	private JCheckBox getCbIsMainImg() {
		if (cbIsMainImg == null) {
			...
			cbIsMainImg.setBounds(new Rectangle(321, 177, 167, 22));
	...
	private JButton getBtnComplex() {
		if (btnComplex == null) {
			...
			btnComplex.setBounds(new Rectangle(466, 50, 21, 24));
			btnComplex.setText("...");
	...
	private JFormattedTextField getTfInnerMergeSeqNum() {
		if (tfInnerMergeSeqNum == null) {
			...
			tfInnerMergeSeqNum.setBounds(new Rectangle(86, 212, 144, 24));
		}
