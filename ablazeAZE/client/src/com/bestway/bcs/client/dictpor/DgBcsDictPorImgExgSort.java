 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class DgBcsDictPorImgExgSort extends JDialogBase {
		...
		if (b) {
			...
			if (isImg) {
				for (int i = 0; i < list.size(); i++) {
					BcsDictPorImg obj = (BcsDictPorImg) list.get(i);
					obj.setSeqNum(i + beginSeqNum);
					listS.add(obj);
				}
			} else {
				for (int i = 0; i < list.size(); i++) {
					BcsDictPorExg obj = (BcsDictPorExg) list.get(i);
					obj.setSeqNum(i + beginSeqNum);
					listS.add(obj);
				}
			}
	...
	class UserListCellRenderer extends DefaultListCellRenderer {
		public Component getListCellRendererComponent(JList list, Object value,
			...
			if (value != null) {
				if (isImg) {
					BcsDictPorImg obj = ((BcsDictPorImg) value);
					s = formatStringByLength(obj.getSeqNum().toString(), 5)
							+ formatStringByLength(
									obj.getComplex() == null ? "" : obj
											.getComplex().getCode(), 20)
							+ formatStringByLength(obj.getCommName(), 50)
							+ formatStringByLength(obj.getCommSpec(), 50);
				} else {
					BcsDictPorExg obj = ((BcsDictPorExg) value);
					s = formatStringByLength(obj.getSeqNum().toString(), 5)
							+ formatStringByLength(
									obj.getComplex() == null ? "" : obj
											.getComplex().getCode(), 20)
	...
	private void move(boolean isup) {
		if (jList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(DgBcsDictPorImgExgSort.this,
					"请选择要移动的项!", "提示", 2);
			return;
		}
		...
		if (isImg) {
			BcsDictPorImg selectinfo = null;
			BcsDictPorImg preinfo = null;
			if (isup) {// 向上移动
				selectinfo = (BcsDictPorImg) selectObj;
				selectinfo.setSeqNum(selectinfo.getSeqNum() - 1);
				preinfo = (BcsDictPorImg) jList.getModel().getElementAt(x - 1);
			...
			BcsDictPorExg selectinfo = null;
			BcsDictPorExg preinfo = null;
			if (isup) {// 向上移动
				selectinfo = (BcsDictPorExg) selectObj;
				selectinfo.setSeqNum(selectinfo.getSeqNum() - 1);
				preinfo = (BcsDictPorExg) jList.getModel().getElementAt(x - 1);
				preinfo.setSeqNum(preinfo.getSeqNum() + 1);
				list.setElementAt(selectinfo, x - 1);
				list.setElementAt(preinfo, x);
				jList.setListData(list);
	...
	private JButton getBtnOk() {
		if (btnOk == null) {
			...
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Vector list = DgBcsDictPorImgExgSort.this.getList();
					bcsDictPorAction.saveBcsDictPorImgExg(new Request(CommonVars
							.getCurrUser()), list);
					isOk = true;
					dispose();
				}
