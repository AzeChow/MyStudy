...
public class FmVFStockTravelingExg extends JInternalFrameBase {
	...
	public FmVFStockTravelingExg() {
		...
		authority.checkStockTravelingExg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						if (getVFSection() == null) {
							JOptionPane.showMessageDialog(FmVFStockTravelingExg.this, "请先选择批次!");
							return;
						}
						if(JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockTravelingExg.this, "确定要清空本次批次的数据吗?",
								"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
							return;
						}else{
							CommonProgress.showProgressDialog(FmVFStockTravelingExg.this);
							CommonProgress.setMessage("系统正在删除,请稍等...");
							disableControls();
							vfVerificationAction.deleteVFStockTravelingExgConverts(request, getVFSection());
							vfVerificationAction.deleteVFStockTravelingExgs(request, getVFSection());
							initConvertJtable(new ArrayList<Object>());
							initRawdataJtable(new ArrayList<Object>());
						}
					} catch (Exception e1) {
						CommonProgress.closeProgressDialog();
						JOptionPane.showMessageDialog(FmVFStockTravelingExg.this,e1.getMessage());
					}finally{
						CommonProgress.closeProgressDialog();
						enableControls();
					}
		...
		if(btnReducedCustomsNumber==null){
			...
			btnReducedCustomsNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new convertVFStockTravelingExgConvert().execute();
		...
		if(btnReducedClearanceNumber==null){
			...
			btnReducedClearanceNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new resolveVFStockTravelingExg().execute();
				}
			});
		}
		...
		if(btnImpInventoryDate==null){
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(vfVerificationAction.isExistsBySection(req, getVFSection(), VFStockTravelingExg.class)){						
					...
					dipdie.setvFBaseStockExg(VFStockTravelingExg.class);
...
//					报关商品名称；报关商品规格；报关单位；这算报关数量；折算系数；归并序号；
	...
	private void queryVFSectionData(){
		...
		List<VFStockTravelingExg> listVFStockExg = vfVerificationAction.findVFStockTravelingExgs(request, getVFSection());
		initRawdataJtable(listVFStockExg);
		List<VFStockTravelingExgConvert> listExgConverts = vfVerificationAction.findVFStockTravelingExgConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
		initConvertJtable(listExgConverts);
		//tabbedPane.setSelectedIndex(0);
		enableControls();
	}

	public class resolveVFStockTravelingExg extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			List<VFStockTravelingExg> exgs =vfVerificationAction.findVFStockTravelingExgs(request, getVFSection());
			VFStockTravelingExg exgss = null;
			...
			List<VFStockTravelingExgConvert> ListExgConvert= vfVerificationAction.resolveVFStockTravelingExgs(request,getVFSection());
			return ListExgConvert;
		}
		
		@Override
		protected void done() {
			List<VFStockTravelingExgConvert>  ListExgConvert;
			try {
				ListExgConvert = (List<VFStockTravelingExgConvert>) get();
				tabbedPane.setSelectedIndex(1);
				initConvertJtable(ListExgConvert);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockTravelingExg.this,e.getMessage());
	...
	public class convertVFStockTravelingExgConvert extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			disableControls();
			List<VFStockTravelingExgConvert> ListExgConvert = new ArrayList<VFStockTravelingExgConvert>();
			String logg =null;
			try {
				logg = vfVerificationAction.convertVFStockTravelingExgConvertsEms(
				...
				if (logg != null && !logg.isEmpty()) {
					...
					DgErrorMessage.show(errs);
					String seqNum = tfseqNum.getText().trim();		
					ListExgConvert = vfVerificationAction.findVFStockTravelingExgConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
				}
			} catch (Exception e) {
				CommonProgress.closeProgressDialog();
	...
	class DeleteDataRunnable extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			try {
				...
				if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockTravelingExg.this, "确定要删除所选择的数据吗?",
				...
				ex.printStackTrace();
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockTravelingExg.this,
