...
public class FmVFStockExg extends JInternalFrameBase {
	...
	public FmVFStockExg() {
		...
		authority.checkStockExg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						if (getVFSection() == null) {
							JOptionPane.showMessageDialog(FmVFStockExg.this, "请先选择批次!");
							return;
						}
						if(JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockExg.this, "确定要清空本次批次的数据吗?",
								"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
							return;
						}else{
							CommonProgress.showProgressDialog(FmVFStockExg.this);
							CommonProgress.setMessage("系统正在删除,请稍等...");
							disableControls();
							vfVerificationAction.deleteVFStockExgConverts(request, getVFSection());
							vfVerificationAction.deleteVFStockExgs(request, getVFSection());
							initConvertJtable(new ArrayList<Object>());
							initRawdataJtable(new ArrayList<Object>());
						}
					} catch (Exception e1) {
						CommonProgress.closeProgressDialog();
						JOptionPane.showMessageDialog(FmVFStockExg.this,e1.getMessage());
					}finally{
						CommonProgress.closeProgressDialog();
						enableControls();
					}
		...
		if(btnImpInventoryDate==null){
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(vfVerificationAction.isExistsBySection(req, getVFSection(), VFStockExg.class)){						
...
//					报关商品名称；报关商品规格；报关单位；这算报关数量；折算系数；归并序号；
	...
	private void queryVFSectionData(){
		...
		List<VFStockExg> listVFStockExg = vfVerificationAction.findVFStockExgs(request, getVFSection());
		initRawdataJtable(listVFStockExg);
		List<VFStockExgConvert> listVFStockExgConverts = vfVerificationAction.findVFStockExgConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
	...
	public class resolveVFStockExg extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			List<VFStockExg> exgs =vfVerificationAction.findVFStockExgs(request, getVFSection());
			VFStockExg exgss = null;
			...
			disableControls();
			List<VFStockExgConvert> ListExgConvert= vfVerificationAction.resolveVFStockExgs(request,getVFSection());
			return ListExgConvert;
		}
		
		@Override
		protected void done() {
			List<VFStockExgConvert>  ListExgConvert;
			try {
				ListExgConvert = (List<VFStockExgConvert>) get();
				tabbedPane.setSelectedIndex(1);
				initConvertJtable(ListExgConvert);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockExg.this,e.getMessage());
	...
	public class convertVFStockExgConvert extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			disableControls();
			List<VFStockExgConvert> ListExgConvert = new ArrayList<VFStockExgConvert>();
			String logg =null;
			try {
				logg = vfVerificationAction.convertVFStockExgConvertsEms(
				...
				if (logg != null && !logg.isEmpty()) {
					...
					DgErrorMessage.show(errs);
					String seqNum = tfseqNum.getText().trim();		
					ListExgConvert = vfVerificationAction.findVFStockExgConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
				}
			} catch (Exception e) {
				CommonProgress.closeProgressDialog();
	...
	class DeleteDataRunnable extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			try {
				...
				if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockExg.this, "确定要删除所选择的数据吗?",
				...
				ex.printStackTrace();
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockExg.this,
