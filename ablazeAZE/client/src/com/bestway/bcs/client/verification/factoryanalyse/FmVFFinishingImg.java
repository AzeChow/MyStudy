...
public class FmVFFinishingImg extends JInternalFrameBase {
	/**
	 * 
	 */
	...
	public FmVFFinishingImg() {
		this.setTitle("在制品原材料");
...
//		authority.checkFinishingImg(new Request(CommonVars.getCurrUser()));
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (getVFSection() == null) {
						JOptionPane.showMessageDialog(FmVFFinishingImg.this, "请先选择批次!");
						return;
					}
					final Request req = new Request(CommonVars.getCurrUser());
					if(vfVerificationAction.isExistsBySection(req, getVFSection(), VFFinishingImg.class)){						
						...
						vfVerificationAction.deleteVFFinishingImgs(request, getVFSection());
						initJtable(new ArrayList<Object>());
					}
					DgImportDataImg dgImportData = new DgImportDataImg();
					dgImportData.setSection(getVFSection());
					dgImportData.setVFBaseStockImg(VFFinishingImg.class);
					dgImportData.setVisible(true);
					FmVFFinishingImg.this.queryVFSectiong();
				}
			});
		}
		...
		if (btnReducedClearanceNumber == null) {
			...
			btnReducedClearanceNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new convertVFFinishingImg().execute();
				}
			});
		}
		...
		if (btnemptyCurrentData == null) {
			...
			btnemptyCurrentData.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFFinishingImg.this, "确定要清空本次批次的数据吗?",
	...
	public class convertVFFinishingImg extends SwingWorker {

		@Override
		protected Object doInBackground() throws Exception {
			CommonProgress.showProgressDialog(FmVFFinishingImg.this);
			CommonProgress.setMessage("系统正在初始化数据，请稍后...");
			String logg;
			List<VFFinishingImg> ListImgConvert = new ArrayList<VFFinishingImg>();
			try {
				String table = "VFFinishingImg";
				...
				if (logg != null && !logg.isEmpty()) {
					...
					ListImgConvert = vfVerificationAction.findVFStockProcessImgsing(request,getVFSection(),(seqNum == null || "".equals(seqNum)) ? null
		...
		@Override
		protected void done() {
			List<VFFinishingImg> VFFinishingImg;
			try {
				VFFinishingImg = (List<VFFinishingImg>) get();
				initJtable(VFFinishingImg);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFFinishingImg.this, e.getMessage());
	...
	public void queryVFSectiong() {
		...
		disableControls();
		String seqNum = tfseqNum.getText().trim();		
		List<VFFinishingImg> listVFFinishingImg = vfVerificationAction.findVFFinishingImgs(request, getVFSection(), ("".equals(seqNum) ?null:Integer.parseInt(seqNum)));
		initJtable(listVFFinishingImg);
		enableControls();
	}
		...
		if (btnDelete == null) {
			...
			btnDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFFinishingImg.this, "确定要删除所选择的数据吗?",
							"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE, null, new Object[] { "是",
									"否" }, "否")) {
						return;
					} else {
