...
public class FmVFStockTravelingImg extends JInternalFrameBase {
	...
	public FmVFStockTravelingImg() {
		...
		authority.checkStockTravelingImg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
	...
	public void initTable(List<VFStockTravelingImg> list) {
		...
		if (btnImport == null) {
			...
			btnImport.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					// 导入盘点数据
					if (checkData()) {						
						if(vfAction.isExistsBySection(new Request(CommonVars.getCurrUser()), vfSection, VFStockTravelingImg.class)){						
							...
							vfAction.deleteVFStockTravelingImgs(request, vfSection);
							initTable(null);
						}
						DgVFStockTravelingImg dgVFStockTravelingImg = new DgVFStockTravelingImg();
						dgVFStockTravelingImg.setVfSection(vfSection);
						dgVFStockTravelingImg.setFmVFStockTravelingImg(FmVFStockTravelingImg.this);
						dgVFStockTravelingImg.setVisible(true);
					}
				}
			});
		}
	...
	public JButton getBtnSwitch() {
		if (btnSwitch == null) {
			btnSwitch = new JButton("2.\u6298\u7B97\u62A5\u5173\u6570\u636E");
	...
	class SwitchWorker extends SwingWorker {
		@Override
		protected List<VFStockTravelingImg> doInBackground() throws Exception {
			List<VFStockTravelingImg> list = null;
			CommonProgress.showProgressDialog(FmVFStockTravelingImg.this);
			CommonProgress.setMessage("系统正在初始化数据，请稍后...");
			String logg;
			List<VFStockTravelingImg> ListImgConvert = new ArrayList<VFStockTravelingImg>();
			try {
				String table = "VFStockTravelingImg";
				...
				if (logg != null && !logg.isEmpty()) {
					...
					ListImgConvert = vfVerificationAction.findVFStockProcessImgslu(request,vfSection,(seqNum == null || "".equals(seqNum)) ? null
		...
		protected void done() {
			try {
				initTable((List<VFStockTravelingImg>) this.get());
		...
		if (btnHistroy == null) {
			...
			btnHistroy.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (checkData()) {
						String seqNum = tfseqNum.getText().trim();
						List<VFStockTravelingImg> dataSource = vfAction.findVFStockTravelingImgs(request, vfSection,  (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
			...
			btnClean.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (checkData() && table.getRowCount() > 0) {
						if (JOptionPane.OK_OPTION == JOptionPane.showOptionDialog(FmVFStockTravelingImg.this,
								"假如导入数据，将删除本批次下的 历史数据！", "提示", JOptionPane.OK_OPTION,
								JOptionPane.INFORMATION_MESSAGE, null, new Object[] { "是", "否" }, "否")) {
							vfAction.deleteVFStockTravelingImgs(request, vfSection);
							initTable(null);
						}
		...
		if (btnDelete == null) {
			...
			btnDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockTravelingImg.this, "确定要删除所选择的数据吗?",
