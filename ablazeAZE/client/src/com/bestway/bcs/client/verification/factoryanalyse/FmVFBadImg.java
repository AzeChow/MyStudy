...
public class FmVFBadImg extends JInternalFrameBase {
	/**
	 * 
	 */
...
//		authority.checkBadImg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (getVFSection() == null) {
						JOptionPane.showMessageDialog(FmVFBadImg.this, "请先选择批次!");
						return;
					}
					final Request req = new Request(CommonVars.getCurrUser());
					if(vfVerificationAction.isExistsBySection(req, getVFSection(), VFBadImg.class)){						
						...
						vfVerificationAction.deleteVFBadImgs(request, getVFSection());
						initJtable(new ArrayList<Object>());
					}
					DgImportDataImg dgImportData = new DgImportDataImg();
					dgImportData.setSection(getVFSection());
					dgImportData.setVFBaseStockImg(VFBadImg.class);
					dgImportData.setVisible(true);
					FmVFBadImg.this.queryVFSectiong();
				}
			});
		}
		...
		if (btnemptyCurrentData == null) {
			...
			btnemptyCurrentData.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFBadImg.this, "确定要清空本次批次的数据吗?",
	...
	public class convertVFBadImg extends SwingWorker {

		@Override
		protected Object doInBackground() throws Exception {
//			CommonProgress.showProgressDialog(FmVFBadImg.this);
//			CommonProgress.setMessage("系统正在折算报关数量，请稍后...");
//			disableControls();
//			List<VFBadImg> VFBadImg = vfVerificationAction.convertVFBadImgs(request, getVFSection());
//			return VFBadImg;
			CommonProgress.showProgressDialog(FmVFBadImg.this);
			CommonProgress.setMessage("系统正在初始化数据，请稍后...");
			String logg;
			List<VFBadImg> ListImgConvert = new ArrayList<VFBadImg>();
			try {
				String table = "VFBadImg";
				...
				if (logg != null && !logg.isEmpty()) {
					...
					ListImgConvert = vfVerificationAction.findVFStockProcessImgsbad(request,getVFSection(),(seqNum == null || "".equals(seqNum)) ? null
		...
		@Override
		protected void done() {
			List<VFBadImg> VFBadImg;
			try {
				VFBadImg = (List<VFBadImg>) get();
				initJtable(VFBadImg);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFBadImg.this, e.getMessage());
	...
	public void queryVFSectiong() {
		...
		disableControls();
		String seqNum = tfseqNum.getText().trim();		
		List<VFBadImg> listVFBadImg = vfVerificationAction.findVFBadImgs(request, getVFSection(), ("".equals(seqNum) ?null:Integer.parseInt(seqNum)));
		initJtable(listVFBadImg);
		enableControls();
	}
		...
		if (btnDelete == null) {
			...
			btnDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFBadImg.this, "确定要删除所选择的数据吗?",
							"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE, null, new Object[] { "是",
									"否" }, "否")) {
						return;
					} else {
