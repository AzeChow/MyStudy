...
public class FmVFBadExg extends JInternalFrameBase {
...
//		authority.checkBadExg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						if (getVFSection() == null) {
							JOptionPane.showMessageDialog(FmVFBadExg.this, "请先选择批次!");
							return;
						}
						if(JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFBadExg.this, "确定要清空本次批次的数据吗?",
								"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
							return;
						}else{
							CommonProgress.showProgressDialog(FmVFBadExg.this);
							CommonProgress.setMessage("系统正在删除,请稍等...");
							disableControls();
							vfVerificationAction.deleteVFBadExgConverts(request, getVFSection());
							vfVerificationAction.deleteVFBadExgs(request, getVFSection());
							initConvertJtable(new ArrayList<Object>());
							initRawdataJtable(new ArrayList<Object>());
						}
					} catch (Exception e1) {
						CommonProgress.closeProgressDialog();
						JOptionPane.showMessageDialog(FmVFBadExg.this,e1.getMessage());
					}finally{
						CommonProgress.closeProgressDialog();
						enableControls();
					}
		...
		if(btnReducedCustomsNumber==null){
			...
			btnReducedCustomsNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new convertVFBadExgConvert().execute();
		...
		if(btnImpInventoryDate==null){
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(vfVerificationAction.isExistsBySection(req, getVFSection(), VFBadExg.class)){						
					...
					dipdie.setvFBaseStockExg(VFBadExg.class);
...
//					报关商品名称；报关商品规格；报关单位；这算报关数量；折算系数；归并序号；
	...
	private void queryVFSectionData(){
		...
		List<VFBadExg> listVFBadExg = vfVerificationAction.findVFBadExgs(request, getVFSection());
		initRawdataJtable(listVFBadExg);
		List<VFBadExgConvert> listVFBadExgConverts = vfVerificationAction.findVFBadExgConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
		initConvertJtable(listVFBadExgConverts);
		//tabbedPane.setSelectedIndex(0);
		enableControls();
	}

	public class resolveVFBadExg extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			List<VFBadExg> exgs =vfVerificationAction.findVFBadExgs(request,getVFSection());
			VFBadExg exgss = null;
			...
			List<VFBadExgConvert> ListExgConvert= vfVerificationAction.resolveVFBadExgs(request,getVFSection());
			return ListExgConvert;
		}
		
		@Override
		protected void done() {
			List<VFBadExgConvert>  ListExgConvert;
			try {
				ListExgConvert = (List<VFBadExgConvert>) get();
				tabbedPane.setSelectedIndex(1);
				initConvertJtable(ListExgConvert);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFBadExg.this,e.getMessage());
	...
	public class convertVFBadExgConvert extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			disableControls();
			List<VFBadExgConvert> ListExgConvert = new ArrayList<VFBadExgConvert>();
			String logg =null;
			try {
				logg = vfVerificationAction.convertVFBadExgConvertsEms(
				...
				String seqNum = tfseqNum.getText().trim();		
				ListExgConvert = vfVerificationAction.findVFBadExgConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
			} catch (Exception e) {
				CommonProgress.closeProgressDialog();
		...
		if (btnDelete == null) {
			...
			btnDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFBadExg.this, "确定要删除所选择的数据吗?",
