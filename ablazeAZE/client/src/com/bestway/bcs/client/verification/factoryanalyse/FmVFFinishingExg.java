...
public class FmVFFinishingExg extends JInternalFrameBase {
	...
	public FmVFFinishingExg() {
		...
		// authority.checkFinishingExg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
					...
					try {
						if (getVFSection() == null) {
							JOptionPane.showMessageDialog(
									FmVFFinishingExg.this, "请先选择批次!");
							return;
						}
						if (JOptionPane.OK_OPTION != JOptionPane
								.showOptionDialog(FmVFFinishingExg.this,
										"确定要清空本次批次的数据吗?", "提示",
							...
							CommonProgress
									.showProgressDialog(FmVFFinishingExg.this);
							CommonProgress.setMessage("系统正在删除,请稍等...");
							disableControls();
							vfVerificationAction.deleteVFFinishingExgConverts(
									request, getVFSection());
							vfVerificationAction.deleteVFFinishingExgs(request,
									getVFSection());
							initConvertJtable(new ArrayList<Object>());
							initRawdataJtable(new ArrayList<Object>());
						}
						...
						JOptionPane.showMessageDialog(FmVFFinishingExg.this,
								e1.getMessage());
					} finally {
						CommonProgress.closeProgressDialog();
						enableControls();
			...
			btnExportExcel.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (tabbedPane.getSelectedIndex() == 0) {
						showTableModelRawdata.getMiSaveTableListToExcel()
								.doClick();
					} else {
						showTableModelConvert.getMiSaveTableListToExcel()
								.doClick();
					}
					enableControls();
				}
		...
		if (btnReducedCustomsNumber == null) {
			...
			btnReducedCustomsNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new convertVFFinishingExgConvert().execute();
		...
		if (btnReducedClearanceNumber == null) {
			...
			btnReducedClearanceNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new ResolveVFFinishingExg().execute();
				}
			});
		}
		...
		if (btnImpInventoryDate == null) {
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (vfVerificationAction.isExistsBySection(req,
							getVFSection(), VFFinishingExg.class)) {
					...
					dipdie.setvFBaseStockExg(VFFinishingExg.class);
	...
	private void initConvertJtable(List list) {
		...
		showTableModelConvert = new AttributiveCellTableModel(
				(MultiSpanCellTable) rawDataTable, list, adapterRawdata);
	...
	private void initRawdataJtable(List list) {
		...
		showTableModelRawdata = new JTableListModel(convertTable, list,
				adapterConvert);
	}
	...
	private void queryVFSectionData() {
		if (getVFSection() == null) {
			JOptionPane.showMessageDialog(FmVFFinishingExg.this, "请先选择批次!");
			return;
		}
		disableControls();
		String seqNum = tfseqNum.getText().trim();
		List<VFFinishingExg> listVFFinishingExg = vfVerificationAction
				.findVFFinishingExgs(request, getVFSection());
		initRawdataJtable(listVFFinishingExg);
		List<VFFinishingExgConvert> listVFFinishingExgConverts = vfVerificationAction
				.findVFFinishingExgConverts(
						request,
						getVFSection(),
						(seqNum == null || "".equals(seqNum)) ? null : Integer
								.parseInt(seqNum));
		initConvertJtable(listVFFinishingExgConverts);
		// tabbedPane.setSelectedIndex(0);
		enableControls();
	}

	public class ResolveVFFinishingExg extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			...
			CommonProgress.showProgressDialog(FmVFFinishingExg.this);
			...
			List<VFFinishingExg> exgs =vfVerificationAction.findVFFinishingExgs(request,getVFSection());
			VFFinishingExg exgss = null;
			...
			List<VFFinishingExgConvert> ListExgConvert = vfVerificationAction
					.resolveVFFinishingExgs(request, getVFSection());
			return ListExgConvert;
		}

		@Override
		protected void done() {
			List<VFFinishingExgConvert> ListExgConvert;
			try {
				ListExgConvert = (List<VFFinishingExgConvert>) get();
				...
				System.out.println("2.成品折算料件（报关常用工厂BOM）共用时："
						+ (finishTime - startTime) / 1000 + "秒,合计为"
	...
	public class convertVFFinishingExgConvert extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			...
			disableControls();
			List<VFFinishingExgConvert> ListExgConvert = new ArrayList<VFFinishingExgConvert>();
			String logg = null;
			try {
				logg = vfVerificationAction.convertVFFinishingExgConvertsEms(
						request, getVFSection());
				...
				String seqNum = tfseqNum.getText().trim();
				ListExgConvert = vfVerificationAction
						.findVFFinishingExgConverts(
								request,
								getVFSection(),
		...
		protected void done() {
			...
			try {
				...
				long finishTime = System.currentTimeMillis();
				System.out.println("3.折算报关数量共用时：" + (finishTime - startTime)
						/ 1000 + "秒,合计为" + ((finishTime - startTime) / 1000)
						/ 1000 + "分钟");
			}
		}
	}
	...
	class DeleteDataRunnable extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			try {
				...
				if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFFinishingExg.this, "确定要删除所选择的数据吗?",
