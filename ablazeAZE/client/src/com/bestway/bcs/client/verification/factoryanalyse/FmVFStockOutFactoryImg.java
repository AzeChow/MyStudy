...
public class FmVFStockOutFactoryImg extends JInternalFrameBase {
	...
	public FmVFStockOutFactoryImg() {
		...
		authority.checkStockOutFactoryImg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
	...
	public void initTable(List<VFStockOutFactoryImg> list) {
			...
			btnImport.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (checkData()) {
						...
						if(vfAction.isExistsBySection(req, vfSection, VFStockOutFactoryImg.class)){						
							...
							vfAction.deleteVFStockOutFactoryImgs(request, vfSection);
							initTable(null);
						}
						DgVFStockOutFactoryImg dgVFStockOutFactoryImg = new DgVFStockOutFactoryImg();
						dgVFStockOutFactoryImg.setVfSection(vfSection);
						dgVFStockOutFactoryImg.setFmVFStockOutFactoryImg(FmVFStockOutFactoryImg.this);
						dgVFStockOutFactoryImg.setVisible(true);
					}
				}
			});
		}
	...
	class SwitchWorker extends SwingWorker {
		@Override
		protected List<VFStockOutFactoryImg> doInBackground() throws Exception {
//				List<VFStockOutFactoryImg> list = null;
				CommonProgress.showProgressDialog(FmVFStockOutFactoryImg.this);
				CommonProgress.setMessage("系统正在初始化数据，请稍后...");
				String logg;
				List<VFStockOutFactoryImg> ListImgConvert = new ArrayList<VFStockOutFactoryImg>();
				try {
					String table ="VFStockOutFactoryImg";
					logg = vfVerificationAction.convertImgConverts(
							request, vfSection,table);
					if (logg != null && !logg.isEmpty()) {
						...
						ListImgConvert = vfVerificationAction.findVFStockProcessImgsout(request, vfSection, (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
					}
//				list = vfAction.convertVFStockOutFactoryImgs(request, vfSection);// 初始化查询条件
		...
		protected void done() {
			try {
				initTable((List<VFStockOutFactoryImg>) this.get());
		...
		if (btnHistroy == null) {
			...
			btnHistroy.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (checkData()) {
						String seqNum = tfseqNum.getText().trim();						
						List<VFStockOutFactoryImg> dataSource = vfAction.findVFStockOutFactoryImgs(request, vfSection,  (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
			...
			btnClean.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (checkData() && table.getRowCount() > 0) {
						if (JOptionPane.OK_OPTION == JOptionPane.showOptionDialog(FmVFStockOutFactoryImg.this,
		...
		if (btnDelete == null) {
			...
			btnDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockOutFactoryImg.this, "确定要删除所选择的数据吗?",
