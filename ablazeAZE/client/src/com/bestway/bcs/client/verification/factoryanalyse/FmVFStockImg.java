...
public class FmVFStockImg extends JInternalFrameBase {
	/**
	 * 
	 */
	...
	public FmVFStockImg() {
		...
		authority.checkStockImg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (getVFSection() == null) {
						JOptionPane.showMessageDialog(FmVFStockImg.this, "请先选择批次!");
						return;
					}
					final Request req = new Request(CommonVars.getCurrUser());
					if(vfVerificationAction.isExistsBySection(req, getVFSection(), VFStockImg.class)){						
						...
						vfVerificationAction.deleteVFStockImgs(request, getVFSection());
						initJtable(new ArrayList<Object>());
					}
					DgImportDataImg dgImportData = new DgImportDataImg();
					dgImportData.setSection(getVFSection());
					dgImportData.setVFBaseStockImg(VFStockImg.class);
					dgImportData.setVisible(true);
					FmVFStockImg.this.queryVFSectiong();
				}
			});
		}
		...
		if (btnemptyCurrentData == null) {
			...
			btnemptyCurrentData.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockImg.this, "确定要清空本次批次的数据吗?",
	...
	public class convertVFStockImg extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			CommonProgress.showProgressDialog(FmVFStockImg.this);
			CommonProgress.setMessage("系统正在折算报关数量，请稍后...");
			disableControls();
			String logg;
			List<VFStockImg> ListImgConvert = new ArrayList<VFStockImg>();
			try {
				String table ="VFStockImg";
				...
				if (logg != null && !logg.isEmpty()) {
					...
					ListImgConvert = vfVerificationAction.findVFStockImgs(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
				}
			} catch (Exception e) {
...
//			
//			List<VFStockImg> VFStockImg = vfVerificationAction.convertVFStockImgs(request, getVFSection());
//			return VFStockImg;
		}

		@Override
		protected void done() {
			List<VFStockImg> VFStockImg;
			try {
				VFStockImg = (List<VFStockImg>) get();
				initJtable(VFStockImg);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockImg.this, e.getMessage());
	...
	public void queryVFSectiong() {
		...
		disableControls();
		String seqNum = tfseqNum.getText().trim();		
		List<VFStockImg> listVFStockImg = vfVerificationAction.findVFStockImgs(request, getVFSection(), ("".equals(seqNum) ?null:Integer.parseInt(seqNum)));
		...
		if (btnDelete == null) {
			...
			btnDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockImg.this, "确定要删除所选择的数据吗?",
							"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE, null, new Object[] { "是",
									"否" }, "否")) {
						return;
					} else {
