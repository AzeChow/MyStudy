...
public class FmVFStockSemiExgHadStore extends JInternalFrameBase {
	...
	public FmVFStockSemiExgHadStore() {
		...
		authority.checkStockSemiExgHadStore(new Request(CommonVars.getCurrUser()));
		initialize();
	}
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						if (getVFSection() == null) {
							JOptionPane.showMessageDialog(FmVFStockSemiExgHadStore.this, "请先选择批次!");
							return;
						}
						if(JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockSemiExgHadStore.this, "确定要清空本次批次的数据吗?",
								"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
							return;
						}else{
							CommonProgress.showProgressDialog(FmVFStockSemiExgHadStore.this);
							CommonProgress.setMessage("系统正在删除,请稍等...");
							disableControls();
							vfVerificationAction.deleteVFStockSemiExgHadStoreConverts(request, getVFSection());
							vfVerificationAction.deleteVFStockSemiExgHadStores(request, getVFSection());
							initConvertJtable(new ArrayList<Object>());
							initRawdataJtable(new ArrayList<Object>());
						}
					} catch (Exception e1) {
						CommonProgress.closeProgressDialog();
						JOptionPane.showMessageDialog(FmVFStockSemiExgHadStore.this,e1.getMessage());
					}finally{
						CommonProgress.closeProgressDialog();
						enableControls();
					}
			...
			btnReducedCustomsNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					SwingUtilities.invokeLater(new Runnable() {
						...
						@Override
						public void run() {
							new convertVFStockSemiExgHadStoreConvert().execute();
						}
					});
		...
		if(btnReducedClearanceNumber==null){
			...
			btnReducedClearanceNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new ResolveVFStockSemiExgHadStore().execute();
				}
			});
		}
		...
		if(btnImpInventoryDate==null){
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(vfVerificationAction.isExistsBySection(req, getVFSection(), VFStockSemiExgHadStore.class)){						
					...
					dipdie.setvFBaseStockExg(VFStockSemiExgHadStore.class);
...
//					报关商品名称；报关商品规格；报关单位；这算报关数量；折算系数；归并序号；
	...
	private void queryVFSectionData(){
		...
		List<VFStockSemiExgHadStore> listVFStockExg = vfVerificationAction.findVFStockSemiExgHadStore(request, getVFSection());
		initRawdataJtable(listVFStockExg);
		List<VFStockSemiExgHadStoreConvert> listVFStockExgConverts = vfVerificationAction.findVFStockSemiExgHadStoreConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
		initConvertJtable(listVFStockExgConverts);
		//tabbedPane.setSelectedIndex(0);
		enableControls();
	}

	public class ResolveVFStockSemiExgHadStore extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			List<VFStockSemiExgHadStore> exgs =vfVerificationAction.findVFStockSemiExgHadStore(request, getVFSection());
			VFStockSemiExgHadStore exgss = null;
			...
			List<VFStockSemiExgHadStoreConvert> ListExgConvert= vfVerificationAction.resolveVFStockSemiExgHadStoreConverts(request,getVFSection());
			return ListExgConvert;
		}
		
		@Override
		protected void done() {
			List<VFStockSemiExgHadStoreConvert>  ListExgConvert;
			try {
				ListExgConvert = (List<VFStockSemiExgHadStoreConvert>) get();
				tabbedPane.setSelectedIndex(1);
				initConvertJtable(ListExgConvert);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockSemiExgHadStore.this,e.getMessage());
	...
	public class convertVFStockSemiExgHadStoreConvert extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			disableControls();
			List<VFStockSemiExgHadStoreConvert> ListExgConvert = new ArrayList<VFStockSemiExgHadStoreConvert>();
			String logg =null;
			try {
				logg = vfVerificationAction.convertVFStockSemiExgHadStoreConvertEms(
				...
				String seqNum = tfseqNum.getText().trim();		
				ListExgConvert = vfVerificationAction.findVFStockSemiExgHadStoreConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
			} catch (Exception e) {
				CommonProgress.closeProgressDialog();
	...
	class DeleteDataRunnable extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			try {
				...
				if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockSemiExgHadStore.this, "确定要删除所选择的数据吗?",
				...
				ex.printStackTrace();
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockSemiExgHadStore.this,
