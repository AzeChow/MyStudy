...
public class DgVFStockOutFactoryImg extends JDialogBase {
	...
	private FmVFStockOutFactoryImg fmVFStockOutFactoryImg;

	public FmVFStockOutFactoryImg getFmVFStockOutFactoryImg() {
		return fmVFStockOutFactoryImg;
	}

	public void setFmVFStockOutFactoryImg(FmVFStockOutFactoryImg fmVFStockOutFactoryImg) {
		this.fmVFStockOutFactoryImg = fmVFStockOutFactoryImg;
	}
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			for (int j = 0; j < line.size(); j++) {
				...
				if (j == 0) {
					...
					if ("".equals(strs[4].toString()) || strs[4] == null) {
						sb.append(" 3.数量不能为空");
	...
	class ImportFileDataRunnable extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			...
			try {
				CommonProgress.showProgressDialog(DgVFStockOutFactoryImg.this);
				...
				JOptionPane.showMessageDialog(DgVFStockOutFactoryImg.this,
			...
			btnInput.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					fileChooser.setFileFilter(new CommonFileFilter(new String[] { "txt", "xls" },
							"选择文档"));
					int state = fileChooser.showOpenDialog(DgVFStockOutFactoryImg.this);
		...
		if (btnSaveData == null) {
			...
			btnSaveData.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (!isPass) {
						JOptionPane.showMessageDialog(DgVFStockOutFactoryImg.this, "有错误信息，不可保存！",
								"提示!", 2);
					...
					if (tableMode.getRowCount() == 0) {
						JOptionPane.showMessageDialog(DgVFStockOutFactoryImg.this, "无数据可保存", "提示!",
								JOptionPane.INFORMATION_MESSAGE);
						return;
					}
					List<VFStockOutFactoryImg> listVF = new ArrayList<VFStockOutFactoryImg>();
					if(lsResult2 == null) lsResult2 = lsResult;
					System.out.println("lsResult2  >> "+lsResult2+"   lsResult  >>>"+lsResult);
					for(int i = 0; i < lsResult2.size(); i++){
							VFStockOutFactoryImg vf = new VFStockOutFactoryImg();
								vf.setPtNo(lsResult2.get(i)[0].toString());
								vf.setPtName(lsResult2.get(i)[1].toString());
								vf.setPtSpec(lsResult2.get(i)[2].toString());
								vf.setPtUnit(lsResult2.get(i)[3].toString());
					...
					// ****保存数据逻辑
					vfAction.saveVFStockOutFactoryImgs(request, vfSection, listVF);

					JOptionPane.showMessageDialog(DgVFStockOutFactoryImg.this, "保存数据成功！", "提示!",
							JOptionPane.INFORMATION_MESSAGE);
					fmVFStockOutFactoryImg.initTable(listVF);
					DgVFStockOutFactoryImg.this.dispose();
