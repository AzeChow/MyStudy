...
public class FmVFStockOutSendSemiExg extends JInternalFrameBase {
	...
	public FmVFStockOutSendSemiExg() {
		
		this.setTitle("外发库存半成品");
		...
		authority.checkStockOutSendSemiExg(new Request(CommonVars.getCurrUser()));
		initialize();
	}
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						if (getVFSection() == null) {
							JOptionPane.showMessageDialog(FmVFStockOutSendSemiExg.this, "请先选择批次!");
							return;
						}
						if(JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockOutSendSemiExg.this, "确定要清空本次批次的数据吗?",
								"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
							return;
						}else{
							CommonProgress.showProgressDialog(FmVFStockOutSendSemiExg.this);
							CommonProgress.setMessage("系统正在删除,请稍等...");
							disableControls();
							vfVerificationAction.deleteVFStockOutSendSemiExgConverts(request, getVFSection());
							vfVerificationAction.deleteVFStockOutSendSemiExgs(request, getVFSection());
							initConvertJtable(new ArrayList<Object>());
							initRawdataJtable(new ArrayList<Object>());
						}
					} catch (Exception e1) {
						CommonProgress.closeProgressDialog();
						JOptionPane.showMessageDialog(FmVFStockOutSendSemiExg.this,e1.getMessage());
					}finally{
						CommonProgress.closeProgressDialog();
						enableControls();
					}
		...
		if(btnReducedCustomsNumber==null){
			...
			btnReducedCustomsNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new convertVFStockExgConvert().execute();
		...
		if(btnImpInventoryDate==null){
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(vfVerificationAction.isExistsBySection(req, getVFSection(), VFStockOutSendSemiExg.class)){						
					...
					dipdie.setvFBaseStockExg(VFStockOutSendSemiExg.class);
...
//					报关商品名称；报关商品规格；报关单位；这算报关数量；折算系数；归并序号；
	...
	private void queryVFSectionData(){
		...
		List<VFStockOutSendSemiExg> listVFStockExg = vfVerificationAction.findVFStockOutSendSemiExgs(request, getVFSection());
		initRawdataJtable(listVFStockExg);
		List<VFStockOutSendSemiExgConvert> listVFStockExgConverts = vfVerificationAction.findVFStockOutSendSemiExgConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
		initConvertJtable(listVFStockExgConverts);
		//tabbedPane.setSelectedIndex(0);
		enableControls();
	}

	public class resolveVFStockExg extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			List<VFStockOutSendExg> exgs =vfVerificationAction.findVFStockOutSendExgs(request, getVFSection());
			...
			List<VFStockOutSendSemiExgConvert> ListExgConvert= vfVerificationAction.resolveVFStockOutSendSemiExgs(request,getVFSection());
			return ListExgConvert;
		}
		
		@Override
		protected void done() {
			List<VFStockOutSendSemiExgConvert>  ListExgConvert;
			try {
				ListExgConvert = (List<VFStockOutSendSemiExgConvert>) get();
				tabbedPane.setSelectedIndex(1);
				initConvertJtable(ListExgConvert);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockOutSendSemiExg.this,e.getMessage());
	...
	public class convertVFStockExgConvert extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			disableControls();
			List<VFStockOutSendSemiExgConvert> ListExgConvert = new ArrayList<VFStockOutSendSemiExgConvert>();
			String logg =null;
			try {
				logg = vfVerificationAction.convertVFStockOutSendSemiExgConvertsEms(
				...
				String seqNum = tfseqNum.getText().trim();		
				ListExgConvert = vfVerificationAction.findVFStockOutSendSemiExgConverts(request, getVFSection(), (seqNum == null || "".equals(seqNum))?null:Integer.parseInt(seqNum));
			} catch (Exception e) {
				CommonProgress.closeProgressDialog();
	...
	class DeleteDataRunnable extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			try {
				...
				if (JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmVFStockOutSendSemiExg.this, "确定要删除所选择的数据吗?",
				...
				ex.printStackTrace();
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmVFStockOutSendSemiExg.this,
