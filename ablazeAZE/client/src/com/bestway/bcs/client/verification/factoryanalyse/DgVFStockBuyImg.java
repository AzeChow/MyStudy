...
import com.progress.common.util.PSCZip;
import java.awt.event.ActionListener;

@SuppressWarnings("unchecked")
public class DgVFStockBuyImg extends JDialogBase {
	...
	private FmVFStockBuyImg fmVFStockBuyImg = null;
	...
	public FmVFStockBuyImg getFmVFStockBuyImg() {
		return fmVFStockBuyImg;
	}

	public void setFmVFStockBuyImg(FmVFStockBuyImg fmVFStockBuyImg) {
		this.fmVFStockBuyImg = fmVFStockBuyImg;
	}
	...
	private Map<String, String[]> initConditionList() {
		...
		if (bcsInnerMergeList == null || bcsInnerMergeList.get(0) == null) {
			...
			for (int i = 0; i < bcsInnerMergeList.size(); i++) {
				...
				strs[2] = bcs[3] == null ? null:bcs[3].toString();
	...
	class ImportFileDataRunnable extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			...
			try {
				CommonProgress.showProgressDialog(DgVFStockBuyImg.this);
				...
				JOptionPane.showMessageDialog(DgVFStockBuyImg.this, "导入数据失败：！" + e.getMessage(),
			...
			btnInput.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					fileChooser.setFileFilter(new CommonFileFilter(new String[] { "txt", "xls" },
							"选择文档"));
					int state = fileChooser.showOpenDialog(DgVFStockBuyImg.this);
			...
			btnSaveData.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (tableMode.getRowCount() == 0) {
						JOptionPane.showMessageDialog(DgVFStockBuyImg.this, "无数据可保存", "提示!",
								JOptionPane.INFORMATION_MESSAGE);
						return;
					}
					ArrayList<VFStockBuyImg> listVF = new ArrayList<VFStockBuyImg>();
					if(lsResult2 == null) lsResult2 = lsResult;
					for(int i = 0; i < lsResult2.size(); i++){
							VFStockBuyImg vf = new VFStockBuyImg();
					...
					// ****保存数据逻辑
					vfAction.saveVFStockBuyImgs(request, vfSection, listVF);

					JOptionPane.showMessageDialog(DgVFStockBuyImg.this, "保存数据成功！", "提示!",
							JOptionPane.INFORMATION_MESSAGE);
					
					fmVFStockBuyImg.initTable(listVF);
					DgVFStockBuyImg.this.dispose();
				}
			});
		}
		return btnSaveData;
	...
	private void initTable(List list) {
		tableMode = new JTableListModel(jTable, list, new JTableListModelAdapter() {
			public List InitColumns() {
				...
				list.add(new JTableListColumn("仓库", "warehouse", 60));
				list.add(new JTableListColumn("备注", "memo", 60));
