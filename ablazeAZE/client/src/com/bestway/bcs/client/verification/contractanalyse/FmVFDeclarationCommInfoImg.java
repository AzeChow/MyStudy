...
public class FmVFDeclarationCommInfoImg extends JInternalFrameBase {
	 ...
	 * Create the frame.
	 */
	public FmVFDeclarationCommInfoImg() {
		super();
		...
		authority.checkDeclarationCommInfoImg(new Request(CommonVars.getCurrUser()));
		initialize();
		initUI();
			...
			btnHistory.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if(cbbVFSection.getSelectedItem()==null){
						JOptionPane.showMessageDialog(FmVFDeclarationCommInfoImg.this, "请选择需要查询的批次!");
						return;
					}
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						if(type==JOptionPane.OK_OPTION){
							...
							vfVerificationAction.delectVFBySection(new Request(CommonVars.getCurrUser()), vf, VFAnalyType.VFCUSTOMS_IMG);
						}else{
							return;
						}
						...
						ex.printStackTrace();
						JOptionPane.showMessageDialog(FmVFDeclarationCommInfoImg.this, "未能成功删除！","警告",JOptionPane.WARNING_MESSAGE);
					}
	...
	private void initUI(){
		...
		List datalist=vfVerificationAction.findVFSectionList(new Request(CommonVars.getCurrUser()));
		List<VFSection> manulist=new ArrayList<VFSection>();
		for (int i = 0; i < datalist.size(); i++) {
			VFSection vfSection=(VFSection) datalist.get(i);
	...
	{	
		...
		if (pnCommonQueryPage == null)
		{
			pnCommonQueryPage = new PnCommonQueryPage()
			{
				...
				{
					return FmVFDeclarationCommInfoImg.this.initTable(dataSource);
				}
						...
						Object value, boolean isLike)
				{
					if(cbbVFSection.getSelectedItem()==null){
						...
						VFSection vf=(VFSection)cbbVFSection.getSelectedItem();
//						String materielType = ((ItemProperty) cbbDate.getSelectedItem())
//								.getCode();
						dataSource = vfVerificationAction.findPageVFByVFSection(
									new Request(CommonVars.getCurrUser()), vf ,index, length, property, value, isLike, VFAnalyType.VFCUSTOMS_IMG);
						...
						String property, Object value, boolean isLike)
				{
					if(cbbVFSection.getSelectedItem()==null){
						...
						VFSection vf=(VFSection) cbbVFSection.getSelectedItem();
						Long count = vfVerificationAction.countVFCustomsImgOrExgBySection(new Request(CommonVars.getCurrUser()), vf, property, value, isLike, VFAnalyType.VFCUSTOMS_IMG);
						return count; 
					}
					return 0L;
				}
	...
	private JTableListModel initTable(List list){
		tableModel=new JTableListModel(tbMaterials, list, new JTableListModelAdapter() {
			...
			public List InitColumns() {
				...
				list.add(addColumn("计量单位", "unit", 100));
	...
	class GetQueryData extends Thread{
		public void run(){
			try {
				CommonProgress.showProgressDialog(FmVFDeclarationCommInfoImg.this);
				...
				//后增加，再读取
				vfVerificationAction.gainVFCustomsImg(new Request(CommonVars.getCurrUser()), vf);
				
				CommonProgress.setMessage("系统正在读取数据，请稍后...");
				pnCommonQueryPage.setInitState();
			}catch(Exception e){
				e.printStackTrace();
				JOptionPane.showMessageDialog(FmVFDeclarationCommInfoImg.this, "获取并保存数据失败！","警告",JOptionPane.WARNING_MESSAGE);
			}finally{
				setButtonState(true);
	 ...
	 * @param integer
	 */
	public void showData(final VFSection section,final  String emsNo,final  Integer seqNum,final Integer impExpType) {
//		new AsynSwingWorker<List<?>>() {
//			protected List<?> submit() {
//				try {
//					CommonProgress.showProgressDialog();
//					CommonProgress.setMessage("正在加载数据,请稍等...");
//					setButtonState(false);
//					return vfVerificationAction.findVFCustomsImg(new Request(CommonVars.getCurrUser()), section,emsNo,seqNum,impExpType);					
//				} finally{
//					setButtonState(true);
//					CommonProgress.closeProgressDialog();
//				}
...
//			protected void success(List<?> result) {
				...
				List list = vfVerificationAction.findVFCustomsImg(new Request(CommonVars.getCurrUser()), section,emsNo,seqNum,impExpType);
				initTable(list);
//				initTable(result);
//			}
//		}.doWork();
