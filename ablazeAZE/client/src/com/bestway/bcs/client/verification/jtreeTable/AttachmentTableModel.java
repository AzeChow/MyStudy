package com.bestway.bcs.client.verification.jtreeTable;
...
public class AttachmentTableModel extends AbstractTreeTableModel implements TreeTableModel {
	...
	//数据
	public List<VFAttachmentManagement> data;
	...
	public AttachmentTableModel(List<String> columnNames,
			List<VFAttachmentManagement> dataVector) {
	...
	//设置数据
	public List<VFAttachmentManagement> getData(){
		return this.data;
	}
		
	public void setData(Vector<VFAttachmentManagement> entityVector) {
		data = entityVector;
	}
	//创建节点 
	private void createNodes(DefaultMutableTreeNode top) {
		for (int i = 0; i < data.size(); i++) {
			VFAttachmentManagement parentAttachment = data.get(i);
			...
			if(parentAttachment.getChildren()!=null){
				...
				if(parentAttachment.getIsHidden()==null||parentAttachment.getIsHidden()==false){
					for (int j = 0; j < parentAttachment.getChildren().size(); j++) {
						VFAttachmentManagement childAttachment = parentAttachment.getChildren().get(j);
	...
	public boolean isCellEditable(Object node, int column) {
		...
		if(column != 0 && isCellEditable == false)
			return false;
		VFAttachmentManagement attachment = (VFAttachmentManagement)getObjectByNode(node);
	...
	public Object getValueAt(Object node, int column) {  //渲染表格 显示数据		
		VFAttachmentManagement attachment = null;
		Object useObj = getObjectByNode(node);
		if (useObj == null) {
			return null;
		}
		if(useObj instanceof BaseEntity){
			attachment=(VFAttachmentManagement) useObj;
		}else{
	...
	public int getChildCount(Object parent) {
		...
		if(o instanceof VFAttachmentManagement){
			VFAttachmentManagement att =((VFAttachmentManagement)o);
			List<VFAttachmentManagement> ch = att.getChildren();
	 ...
	 public boolean isLeaf(Object node) {
		...
		if(o instanceof VFAttachmentManagement){
			String pid = ((VFAttachmentManagement)getObjectByNode(node)).getParentId();
		    return pid != null;
		}
