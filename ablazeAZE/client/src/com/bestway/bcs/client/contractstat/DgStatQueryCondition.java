...
public class DgStatQueryCondition extends JDialogBase {
	...
	private void queryData() {
		...
		if(impExpFlag!=2){
			lsResult = contractStatAction.findMaterialImportListByContracts(
					new Request(CommonVars.getCurrUser()), isImport, seqNum,
					commCode, name, customer, impExpType, beginDate, endDate,
					contracts, state,impExpFlag);			
		}else{
			lsResult = contractStatAction.findSpecialCustomsDeclarationCommInfo(new Request(CommonVars.getCurrUser()),
	...
	private JButton getBtCancel() {
		if (btCancel == null) {
			...
			btCancel.setBounds(210, 225, 90, 25);
			btCancel.setText("取消");
