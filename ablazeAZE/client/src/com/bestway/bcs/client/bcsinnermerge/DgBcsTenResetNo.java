 ...
 */
public class DgBcsTenResetNo extends JDialogBase {
...
//		List list = bcsInnerMergeAction.findBcsInnerMergeInContract(
...
//		}
	...
	private void tenResetData(int toRowNumber) {
		try {
			...
			CommonProgress.showProgressDialog(DgBcsTenResetNo.this);
			CommonProgress.setMessage("正在进行十位数据重排,请稍候...");
			this.bcsInnerMergeAction.reSortMergeTenNo(new Request(CommonVars
	...
	public int checkDataForTenInnerMergeSort(List selectedRows, int toRowNumber) {
		...
		type = ((BcsInnerMerge) selectedRows.get(0)).getMaterielType();
		maxValue = this.bcsInnerMergeAction.getMaxBcsInnerMergeNo(new Request(
				CommonVars.getCurrUser()), type);
		for (int i = 0; i < selectedRows.size(); i++) {
			BcsInnerMerge data = (BcsInnerMerge) selectedRows.get(i);
			if (data.getBcsTenInnerMerge() == null) {
				return -3;
			}
			int currentValue = data.getBcsTenInnerMerge().getSeqNum().intValue();
			if (i == 0) {
				max = currentValue;
				min = max;
				continue;
