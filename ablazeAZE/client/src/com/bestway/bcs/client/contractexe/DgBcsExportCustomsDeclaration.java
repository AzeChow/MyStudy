...
public class DgBcsExportCustomsDeclaration extends
	...
	public DgBcsExportCustomsDeclaration() {
		super();
		lbEmsNo.setText("备案号");
	...
	protected void addFromMateriel() {
		...
		dg.setMaterialType(MaterielType.FINISHED_PRODUCT);
	...
	protected DgBaseExportCustomsDeclarationCommInfo getExportCustomsDeclarationCommInfo() {
		...
		DgBcsExportCustomsDeclarationCommInfo dg = new DgBcsExportCustomsDeclarationCommInfo();
	...
	protected boolean saveCustom() {
		...
		if (declareDate != null) {
			if (declareDate.compareTo(beginDate) < 0) {
				JOptionPane.showMessageDialog(
						DgBcsExportCustomsDeclaration.this, "申报日期不能小于合同开始生效日期",
		...
		if (endDate != null) {
			if (declareDate != null && declareDate.compareTo(endDate) > 0) {
				JOptionPane.showMessageDialog(
						DgBcsExportCustomsDeclaration.this, "申报日期不能大于合同有效日期",
						"提示", JOptionPane.INFORMATION_MESSAGE);
				return false;
			}
		}
