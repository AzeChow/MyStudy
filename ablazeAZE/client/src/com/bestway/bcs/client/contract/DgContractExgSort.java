...
public class DgContractExgSort extends JDialogBase {
	...
	public void setVisible(boolean b) {
		if (b) {
			...
			for (int i = 0; i < list.size(); i++) {
				ContractExg obj = (ContractExg) list.get(i);
	...
	private void move(boolean isup) {
		if (jList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(DgContractExgSort.this, "请选择要移动的项!",
					"提示", 2);
			return;
		}
		...
		ContractExg selectinfo = null;
		ContractExg preinfo = null;
		if (isup) {// 向上移动
			selectinfo = (ContractExg) selectObj;
			selectinfo.setSeqNum(selectinfo.getSeqNum() - 1);
			preinfo = (ContractExg) jList.getModel().getElementAt(x - 1);
	...
	private JButton getBtnOk() {
		if (btnOk == null) {
			...
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Vector list = DgContractExgSort.this.getList();
					contractAction.saveContractExg(new Request(CommonVars
							.getCurrUser()), list);
					isOk = true;
					dispose();
				}
