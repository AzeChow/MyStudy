 ...
 */
public class DgContractImgSort extends JDialogBase {
	...
	public void setVisible(boolean b) {
		if (b) {
			...
			for (int i = 0; i < list.size(); i++) {
				ContractImg obj = (ContractImg) list.get(i);
	...
	private void move(boolean isup) {
		if (jList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(DgContractImgSort.this, "请选择要移动的项!",
					"提示", 2);
			return;
		}
		...
		ContractImg selectinfo = null;
		ContractImg preinfo = null;
		if (isup) {// 向上移动
			selectinfo = (ContractImg) selectObj;
			selectinfo.setSeqNum(selectinfo.getSeqNum() - 1);
			preinfo = (ContractImg) jList.getModel().getElementAt(x - 1);
	...
	private JButton getBtnOk() {
		if (btnOk == null) {
			...
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Vector list = DgContractImgSort.this.getList();
					contractAction.saveContractImg(new Request(CommonVars
							.getCurrUser()), list);
					isOk = true;
					dispose();
				}
