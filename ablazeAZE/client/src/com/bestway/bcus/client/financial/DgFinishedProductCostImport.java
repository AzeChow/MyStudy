...
public class DgFinishedProductCostImport extends JDialogBase {
	 ...
	 */
	public DgFinishedProductCostImport() {
		super();
	...
	class ImportFileDataRunnable extends Thread {
		public void run() {
			...
			try {
				CommonProgress.showProgressDialog(DgFinishedProductCostImport.this);
	...
	private List parseTxtFile() {
		...
		ArrayList<TempProductAccount> list = new ArrayList<TempProductAccount>();
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			TempProductAccount obj = new TempProductAccount();
			String err = "";
			productAccount.setAccountDate(this.importMonth);
			...
			for (int j = 0; j < zcount; j++) {
				...
				if ("entity.hsName".equals(columnField)) {
					if (value != null && !value.equals("")) {
						productAccount.setHsName(value);
					} else {
						err = err + "[" + value + "]   成品名称为空/";
					}
				} else if ("entity.hsSpec".equals(columnField)) {
					if (value != null && !value.equals("")) {
						productAccount.setHsSpec(value);
					}
					else {
					...
					if (value != null && !value.equals("")) {
						if(unitMap.containsKey(value)){
							productAccount.setHsUnit((Unit)unitMap.get(value));
						}else{
					...
					if (value != null && !value.equals("")) {
						if(FinancialCommon.isDouble(value)){
							productAccount.setThisInCount(Double.parseDouble(value));
						}else{
					...
					if (value != null && !value.equals("")) {
						if(FinancialCommon.isDouble(value)){
							productAccount.setThisInAmount(Double.parseDouble(value));
						}else{
		...
		if (btnSave == null) {
			...
			btnSave.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (list.size() == 0) {
						JOptionPane.showMessageDialog(DgFinishedProductCostImport.this, "数据为空，请先导入数据！", "提示!", 2);
						return;
					}
					...
					for (int i = 0; i < tableList.size(); i++) {
						TempProductAccount obj = (TempProductAccount) tableList.get(i);
						if (obj.getErrinfo() != null
								&& !obj.getErrinfo().equals("")) {
							JOptionPane.showMessageDialog(DgFinishedProductCostImport.this,
									"有错误信息，不可保存！", "提示!", 2);
							return;
						}
					}
	...
	class SaveDataRunnable extends Thread {
		...
		public void run() {
			...
			CommonProgress.setMessage("系统正在读取文件资料，请稍后...");
			try {
				financialAction.importFinishedProductCost(new Request(CommonVars.getCurrUser()), data);
			} catch (Exception e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(DgFinishedProductCostImport.this,
			...
			DgFinishedProductCostImport.this.dispose();
		}
	}
