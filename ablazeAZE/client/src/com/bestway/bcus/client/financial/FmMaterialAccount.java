 ...
 * 
 */
public class FmMaterialAccount extends JInternalFrameBase {
	...
	private JTableListModelAdapter jTableListModelAdapter = null;  //  @jve:decl-index=0:
	...
	private void initialize() {
		this.setTitle("原材料进出仓帐");
		this.setSize(833, 541);
	...
	private JComboBox getJComboBox() {
		if (cmbBillType == null) {
			...
			cmbBillType.setBounds(69, 45, 226, 20);
		}
		return cmbBillType;
	}
	/**
	...
	private JPanel getPnTop() {
		if (pnTop == null) {
			...
			jLabel101.setBounds(new Rectangle(12, 45, 52, 20));
			jLabel101.setText("数据来源");
...
//			pnTop.add(getBtnPrint(), null);
	...
	private JButton getJButton() {
		if (btnQuery == null) {
			...
			btnQuery.setText("查询");
			btnQuery.setBounds(300, 44, 65, 22);
	...
	private void fillBillType(String materialType) {
		...
		List billTypes = financialAction.findBillType(new Request(CommonVars.getCurrUser()), FinancialCommon.FINANCIAL_TYPE_MATERIAL_ACCOUNT);
		...
		for (int i = 0; i < billTypes.size(); i++) {
			...
			if(billType.getCode().equals("1003") //			默认 直接进口
					|| billType.getCode().equals("1004")//	默认 结转进口
					|| billType.getCode().equals("1102")//	默认 料件退换
					|| billType.getCode().equals("1103")){//默认 料件复出
	...
	private void setCmbBillTypeEvent(final JComboBox cbb) {
		...
		tf.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent e) {
				...
				String[] strs = tf.getText().split(",");
				Map<String, CheckBoxListItemForFinancial> CheckBoxListItemForFinancialMap = new HashMap<String, CheckBoxListItemForFinancial>();

				int size = listBox.getModel().getSize();
				for (int i = 0; i < size; i++) {
					CheckBoxListItemForFinancial item = (CheckBoxListItemForFinancial) listBox
							.getModel().getElementAt(i);
					CheckBoxListItemForFinancialMap.put(item.getName(), item);
				}
				//
				// 根据输入的字符串选中JList中的列表
				//
				String tempText = "";
				for (String str : strs) {
					CheckBoxListItemForFinancial item = CheckBoxListItemForFinancialMap.get(str);
					if (item != null) {
						...
						CheckBoxListItemForFinancialMap.remove(str);
					}
				}
				//
				// 清除未选中的记录
				//
				Iterator<CheckBoxListItemForFinancial> iterator = CheckBoxListItemForFinancialMap
						.values().iterator();
				for (; iterator.hasNext();) {
					CheckBoxListItemForFinancial CheckBoxListItemForFinancial = iterator.next();
					CheckBoxListItemForFinancial.setIsSelected(false);
				}
				//
				// 重新设置其显示文本值
				//
	...
	class searchThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				impExpInfos = financialAction.findMaterialAccountList(new Request(CommonVars.getCurrUser()), conditions);
				
				CommonProgress.closeProgressDialog(flag);
			} catch (Exception e) {
				CommonProgress.closeProgressDialog(flag);
				JOptionPane.showMessageDialog(FmMaterialAccount.this, "获取数据失败：！" + e.getMessage(), "提示", 2);
			} finally {
				...
				if (impExpInfos != null && !impExpInfos.isEmpty()) {
					...
					initTable(new ArrayList());
					JOptionPane.showMessageDialog(FmMaterialAccount.this,
							"没有查到符合条件的记录！", "提示！", 2);
				}
			}
	...
	private JButton getBtnPrint() {
		if (btnPrint == null) {
			...
			btnPrint.setText("打印");
			btnPrint.setBounds(655, 44, 65, 22);
	...
	private JButton getJButton2() {
		if (btnExit == null) {
			btnExit = new JButton();
			btnExit.setText("关闭");
			btnExit.setBounds(655, 44, 65, 22);
...
//			detail.setUnitUsed(unitUsed);
	...
	private void initTable(final List list) {
		jTableListModelAdapter = new JTableListModelAdapter(maximumFractionDigits) {
			public List<JTableListColumn> InitColumns() {
				...
				list.add(addColumn("数量", "thisOutCount", 80));
				list.add(addColumn("总金额", "thisOutAmount", 80));
	...
	private void commitValueChange(final JFormattedTextField tf) {
		...
		MaterialAccount materialAccount = (MaterialAccount) tableModel.getCurrentRow();
		...
		materialAccount.setIsSelected(Boolean.TRUE);
	...
	private JButton getBtnReQuery() {
		if (btnReQuery == null) {
			...
			btnReQuery.setBounds(new Rectangle(370, 44, 90, 22));
	...
	private JButton getBtnInput() {
		if (btnInput == null) {
			...
			btnInput.setBounds(new Rectangle(535, 44, 115, 22));
			btnInput.setText("导入修改数据");
			btnInput.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					DgMaterialAccountImport dgImport = new DgMaterialAccountImport();
					if(cbbStartDate.getDate() == null){
						JOptionPane.showMessageDialog(FmMaterialAccount.this, "导入前，请选择月份", "提示", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
	...
	private JButton getSave() {
		if (btnSave == null) {
			btnSave = new JButton();
			btnSave.setText("保存");
			btnSave.setBounds(465, 44, 65, 22);
	...
	private List getUpdateDataList(){
		...
		for(int i = 0 ; i < oriList.size() ; i ++){
			MaterialAccount materialAccount = (MaterialAccount) oriList.get(i);
			if(materialAccount.getIsSelected())
				list.add(materialAccount);
		}
	...
	class saveThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				financialAction.updateMaterialAccount(new Request(CommonVars.getCurrUser()), list);
				CommonProgress.closeProgressDialog(flag);
				isOk = true;
			} catch (Exception e) {
				CommonProgress.closeProgressDialog(flag);
