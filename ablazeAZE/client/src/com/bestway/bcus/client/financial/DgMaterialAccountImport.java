 ...
 * 
 */
public class DgMaterialAccountImport extends JDialogBase {
	 ...
	 */
	public DgMaterialAccountImport() {
		super();
	...
	class ImportFileDataRunnable extends Thread {
		public void run() {
			...
			try {
				CommonProgress.showProgressDialog(DgMaterialAccountImport.this);
	...
	private List parseTxtFile() {
		...
		ArrayList<TempMaterialAccount> list = new ArrayList<TempMaterialAccount>();
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			MaterialAccount materialAccount = new MaterialAccount();
			TempMaterialAccount obj = new TempMaterialAccount();
			String err = "";
			materialAccount.setAccountDate(this.importMonth);
			...
			for (int j = 0; j < zcount; j++) {
				...
				if ("entity.hsName".equals(columnField)) {
					if (value != null && !value.equals("")) {
						materialAccount.setHsName(value);
					} else {
						err = err + "[" + value + "]   料件名称为空/";
					}
				} else if ("entity.hsSpec".equals(columnField)) {
					if (value != null && !value.equals("")) {
						materialAccount.setHsSpec(value);
					}
					else {
						err = err + "[" + value + "]   料件规格为空/";
					}
				} else if ("entity.hsUnit.name".equals(columnField)) {
					if (value != null && !value.equals("")) {
						if(unitMap.containsKey(value)){
							materialAccount.setHsUnit((Unit)unitMap.get(value));
						}else{
							err = err + "[" + value + "]   料件单位不合法/";
						}
					} else {
						err = err + "[" + value + "]   料件单位为空/";
					}
				} else if ("entity.thisInCount".equals(columnField)) {
					if (value != null && !value.equals("")) {
						if(FinancialCommon.isDouble(value)){
							materialAccount.setThisInCount(Double.parseDouble(value));
						}else{
							err = err + "[" + value + "]   本月进入数量不合法/";
						}
					} else {
						 err = err + "[" + value + "]   本月进入数量为空/";
					}
				} else if ("entity.thisInAmount".equals(columnField)) {
					if (value != null && !value.equals("")) {
						if(FinancialCommon.isDouble(value)){
							materialAccount.setThisInAmount(Double.parseDouble(value));
						}else{
							err = err + "[" + value + "]   本月进入金额不合法/";
						}
					} else {
						err = err + "[" + value + "]   本月进入金额为空/";
					}
				}
			}
			obj.setEntity(materialAccount);
			obj.setErrinfo(err);
			list.add(obj);
		}
		return list;
		...
		if (btnSave == null) {
			...
			btnSave.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (list.size() == 0) {
						JOptionPane.showMessageDialog(DgMaterialAccountImport.this, "数据为空，请先导入数据！", "提示!", 2);
						return;
					}
					...
					for (int i = 0; i < tableList.size(); i++) {
						TempMaterialAccount obj = (TempMaterialAccount) tableList.get(i);
						if (obj.getErrinfo() != null
								&& !obj.getErrinfo().equals("")) {
							JOptionPane.showMessageDialog(DgMaterialAccountImport.this,
									"有错误信息，不可保存！", "提示!", 2);
							return;
						}
					}
	...
	class SaveDataRunnable extends Thread {
		...
		public void run() {
			...
			CommonProgress.setMessage("系统正在读取文件资料，请稍后...");
			try {
				financialAction.importMaterialAccount(new Request(CommonVars.getCurrUser()), data);
			} catch (Exception e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(DgMaterialAccountImport.this,
						"导入失败!\n\n数据: " + String.valueOf(data.size()), "提示",
						2);
				e.printStackTrace();
				return;
			} finally {
			...
			CommonProgress.closeProgressDialog();
			JOptionPane.showMessageDialog(DgMaterialAccountImport.this,
					"导入成功!\n\n数据: " + String.valueOf(data.size()), "提示", 2);
			DgMaterialAccountImport.this.dispose();
		}
	}
	...
	private List getDefaultBomTableColumnList() {
		...
		list.add(new JTableListColumn("料件名称", "entity.hsName", 150));
		list.add(new JTableListColumn("料件规格", "entity.hsSpec", 150));
		list.add(new JTableListColumn("料件单位", "entity.hsUnit.name", 80));
		list.add(new JTableListColumn("本月进入数量", "entity.thisInCount", 80));
		list.add(new JTableListColumn("本月进入金额", "entity.thisInAmount", 80));
