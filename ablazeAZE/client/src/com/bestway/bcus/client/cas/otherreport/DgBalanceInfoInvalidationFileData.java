...
public class DgBalanceInfoInvalidationFileData extends JDialogBase {
	 ...
	 *  
	 */
	public DgBalanceInfoInvalidationFileData() {
		super();
		initialize();
		...
		if (btnExport == null) {
			...
			btnExport.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					int state = fileChooser.showSaveDialog(DgBalanceInfoInvalidationFileData.this);
	...
	private void saveListToFile(List dataSource,String filePath){
		...
		try
		{
			...
			for(int i=0;i<dataSource.size();i++){
				BalanceInfo data =(BalanceInfo)dataSource.get(i);
				StringBuilder strLine = new StringBuilder("");
				strLine.append((data.getName()==null?"":data.getName())+"/"+
						(data.getSpec()==null?"":data.getSpec())+"/"+
						(data.getUnitName()==null?"":data.getUnitName())+strTab
						);
				strLine.append(data.getF1().toString()+strTab);
				strLine.append(data.getF2().toString()+strTab);
				strLine.append(data.getF3().toString()+strTab);
				strLine.append(data.getF4().toString()+strTab);
				strLine.append(data.getF5().toString()+strTab);
				strLine.append(data.getF6().toString()+strTab);
		...
		tableModel = new JTableListModel((GroupableHeaderTable) jTable,
				dataSource, new JTableListModelAdapter() {
					public List InitColumns() {
						...
						list.add(addColumn("3.库存原材料合计", "f2", 120));
						list.add(addColumn("1A.库存原料", "f1", 100));
						list.add(addColumn("2A.半成品折原料", "f2", 120));
						list.add(addColumn("3A.成品折原料", "f3", 100));
