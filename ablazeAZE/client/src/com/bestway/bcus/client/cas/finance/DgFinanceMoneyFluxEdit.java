 ...
 */
public class DgFinanceMoneyFluxEdit extends JDialogBase {
	...
	private MoneyMaster moneyMaster = null;
    ...
    public void setVisible(boolean isFlag){
        if(isFlag){
            moneyMaster = (MoneyMaster) tableModelMaster.getCurrentRow();
               list = casAction.findMoneyDetail(new Request(CommonVars.getCurrUser()),moneyMaster);
		...
		tableModel = new JTableListModel(jTable, list,
				new JTableListModelAdapter() {
					public List InitColumns() {
						...
						list.add(addColumn("项目", "item1", 320));
						list.add(addColumn("行次", "times1", 30));
						list.add(addColumn("金额", "money1", 60));
						
						list.add(addColumn("项目", "item2", 200));
						list.add(addColumn("行次", "times2", 30));
						list.add(addColumn("金额", "money2", 60));
						
						list.add(addColumn("补充资料", "item3", 330));
						list.add(addColumn("行次", "times3", 30));
						list.add(addColumn("金额", "money3", 60));
						return list;
					}
				});
	}
	 ...
	 */
	private void initUI(){
		this.jLabel1.setText(CommonVars.dateToString(moneyMaster.getReportDate()));
	}
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			...
			jLabel.setBounds(260, 3, 171, 25);
			jLabel.setText("企  业  会  计  报  表");
			jLabel.setFont(new java.awt.Font("Dialog", java.awt.Font.BOLD, 18));
			jLabel2.setBounds(307, 26, 83, 14);
			jLabel2.setText("现 金 流 量 表");
		...
		if (jButton == null) {
			...
			jButton.addActionListener(new java.awt.event.ActionListener() { 
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {    
					...
					try{
					  DgFinanceMoneyFluxEditEdit dg = new DgFinanceMoneyFluxEditEdit();
			...
			jButton1.addActionListener(new java.awt.event.ActionListener() { 
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {    
					...
					if (list !=null && !list.isEmpty()){
				    	...
				    	InputStream masterReportStream = DgFinanceMoneyFluxEdit.class
			    			.getResourceAsStream("report/FinanceMoneyReport.jasper");			
				    	InputStream detailReportStream = DgFinanceMoneyFluxEdit.class
					    	.getResourceAsStream("report/FinanceMoneyReportSubb.jasper");
					try {
						...
						String defaultDate = bartDateFormat.format(moneyMaster.getReportDate());
						
						Map parameters = new HashMap();
						parameters.put("imgExgDS",imgExgDS);
						parameters.put("detailReport",detailReport);
						parameters.put("riqi",defaultDate); //DgFinanceMoneyFluxEdit.this.jLabel1.getText()
		...
		if (jButton2 == null) {
			...
			jButton2.addActionListener(new java.awt.event.ActionListener() { 
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {    
					...
					DgFinanceMoneyFluxEdit.this.dispose();
				}
			});
