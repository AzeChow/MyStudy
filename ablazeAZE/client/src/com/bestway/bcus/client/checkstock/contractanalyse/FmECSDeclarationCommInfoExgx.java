 ...
 */
public class FmECSDeclarationCommInfoExgx extends JInternalFrameBase {
	 ...
	 */
	public FmECSDeclarationCommInfoExgx() {
		super();
	...
	private JButton getBtnClose() {
		if (btnClose == null) {
			...
			btnClose.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					FmECSDeclarationCommInfoExgx.this.dispose();
				}
			});
		}
		return btnClose;
		...
		if (btnQry == null) {
			...
			btnQry.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new AsynSwingWorker<List<ECSDeclarationCommInfoExg>>() {
						Request request = new Request(CommonVars.getCurrUser());
						protected List<ECSDeclarationCommInfoExg> submit() {				
							long t = System.currentTimeMillis();
							try{
								setButtonState(false);
								CommonProgress.showProgressDialog(FmECSDeclarationCommInfoExgx.this);
								...
								return checkStockAction.findECSDeclarationExgBySectionAndVersion(request, section, seqNum,version);
							}finally{							
						...
						protected void success(List<ECSDeclarationCommInfoExg> list) {
							initTable(list);						
						};						
					}.doWork();
				}
		...
		if (btnDel == null) {
			...
			btnDel.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(JOptionPane.YES_OPTION == JOptionPane.showOptionDialog(FmECSDeclarationCommInfoExgx.this, "确定删除本批次数据","提示",JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
						new AsynSwingWorker<Object>() {
							...
							protected Object submit() {
								try{
									...
									checkStockAction.deleteECSDeclarationCommExgInfoBySection(request, section);
								}finally{
									CommonProgress.closeProgressDialog();
									setButtonState(true);
								}
		...
		if (btnCompute == null) {
			...
			btnCompute.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(checkStockAction.isExistECSDeclarationCommInfoExgBySection(req, section)){
						CommonProgress.closeProgressDialog();
						if(JOptionPane.YES_OPTION != JOptionPane.showOptionDialog(FmECSDeclarationCommInfoExgx.this, "已存在本批次数据，确定删除本批次数据，重新计算?","提示",JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
							return;
						}
					}						
					new AsynSwingWorker<List<ECSDeclarationCommInfoExg>>(){
						...
						protected List<ECSDeclarationCommInfoExg> submit() {
							try{	
								...
								setButtonState(false);
								checkStockAction.deleteECSDeclarationCommExgInfoBySection(req, section);
								return checkStockAction.finddeclarationCommInfoExg(req,section);
							}finally{
								setButtonState(true);
								CommonStepProgress.closeStepProgressDialog();
							}
						}
						protected void success(java.util.List<ECSDeclarationCommInfoExg> e) {
							JOptionPane.showMessageDialog(FmECSDeclarationCommInfoExgx.this,"完成统计！耗时:"+(System.currentTimeMillis()-t)+"毫秒","提示", JOptionPane.INFORMATION_MESSAGE);
							initTable(e);
						};
					}.doWork();
				}
	...
	private void initTable(List list) {
		...
		if(tableModel == null){
			tableModel = new JTableListModel(tbBgImg, list,new JTableListModelAdapter() {
				public List<JTableListColumn> InitColumns() {
					...
					list.add(addColumn("成品名称", "commName", 120));
		...
		tbBgImg.getColumnModel().getColumn(12).setCellRenderer(
	 ...
	 * @param seqNum
	 */
	public void showData(ECSSection section,Integer seqNum,String version){
		setSection(section);
...
//					String seqNum = tfSeqNum.getText().trim();
					return checkStockAction
							.finddeclarationCommInfoExg(req,section);
				}finally{
					CommonProgress.closeProgressDialog();
//					initBtnStatus(2);
				}
