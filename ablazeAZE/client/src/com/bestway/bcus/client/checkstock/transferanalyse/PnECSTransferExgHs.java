...
public class PnECSTransferExgHs extends JPanel{
	...
	public PnECSTransferExgHs(FmECSTransferExgBalance parent) {
		this.parent = parent;
		...
		if(adapterRawdata==null){
			adapterRawdata = new JTableListModelAdapter() {
				public List<JTableListColumn> InitColumns() {
					...
					list.add(addColumn("成品已送货未转厂", "baseExg.hsUnTransferNum", 110));
					list.add(addColumn("成品已转厂未送货", "baseExg.hsUnSendferNum", 110));
					...
					list.add(addColumn("已送货未转厂", "hsUnTransferNum", 100));
					list.add(addColumn("已转厂未送货", "hsUnSendferNum", 100));					
					return list;
				}
			};
		}
	...
	JTableListModel initJtableP(List<?> list){
		if(adapterConvert==null){
			adapterConvert = new JTableListModelAdapter() {
				public List<JTableListColumn> InitColumns() {
					...
					list.add(addColumn("已送货未转厂", "hsUnTransferNum", 100));
					list.add(addColumn("已转厂未送货", "hsUnSendferNum", 100));
					list.add(addColumn("送货数量", "hsBillNum", 100));
		...
		hsImpGroup.add(cm.getColumn(1));
		hsImpGroup.add(cm.getColumn(2));
		hsImpGroup.add(cm.getColumn(3));
		hsImpGroup.add(cm.getColumn(4));
		hsImpGroup.add(cm.getColumn(5));
				...
				public void actionPerformed(ActionEvent e) {
					if(parent.getEcsSection() == null){
						...
						try{
							...
							DgECSImport dgECSImport = new DgECSImport(false,true);
		...
		if (btnImpFormFpt == null) {
			...
			btnImpFormFpt.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(PnECSTransferExgHs.this, "确定先删除本批次数据，并从深加工模块获取数据吗？",
							"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
						return ;
					}					
					new AsynSwingWorker<List<ECSTransferDiffExg> >() {
						
						public List<ECSTransferDiffExg> submit() {			
							...
							try{
								initBtnStatus(0);
								return parent.ecsCheckStockAction.generateTransferDiffExgHsFormFpt(req, section);												
							}finally{
						...
						protected void success(List<ECSTransferDiffExg> e) {
							tabbedPane.setSelectedIndex(0);
							initJtableP(e);
							initJtableH(Collections.EMPTY_LIST);
						}
