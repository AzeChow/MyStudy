...
public class FmECSSemiFinishedExg extends JInternalFrameBase {
	...
	public FmECSSemiFinishedExg() {
		...
		authority.checkSemiFinishedExg(request);
		initialize();
	}
		...
		if(btnemptyCurrentData==null){
			...
			btnemptyCurrentData.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						if (getSection() == null) {
							JOptionPane.showMessageDialog(FmECSSemiFinishedExg.this, "请先选择批次!");
							return;
						}
						if(JOptionPane.OK_OPTION != JOptionPane.showOptionDialog(FmECSSemiFinishedExg.this, "确定要清空本次批次的数据吗?",
								"提示", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE,null, new Object[]{"是","否"},"否")){
							return;
						}else{
							CommonProgress.showProgressDialog(FmECSSemiFinishedExg.this);
							CommonProgress.setMessage("系统正在删除,请稍等...");
							disableControls();
							ecsCheckStockAction.deleteECSSemiFinishedExg(request, getSection());
							initConvertJtable(new ArrayList<Object>());
							initRawdataJtable(new ArrayList<Object>());
						}
					} catch (Exception e1) {
						CommonProgress.closeProgressDialog();
						JOptionPane.showMessageDialog(FmECSSemiFinishedExg.this,e1.getMessage());
					}finally{
						CommonProgress.closeProgressDialog();
						enableControls();
					}
	...
	private JButton getBtnViewHistory(){
		if(btnViewHistory==null){
			...
			btnViewHistory.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					FmECSSemiFinishedExg.this.doFindStars();
				}
			});
		}
		...
		if(btnReducedCustomsNumber==null){
			...
			btnReducedCustomsNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					FmECSSemiFinishedExg.this.doConvertStars();
				}
			});
		}
		...
		if(btnReducedClearanceNumber==null){
			...
			btnReducedClearanceNumber.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					new resolveECSSemiFinishedExg().execute();
				}
			});
		}
		...
		if(btnImpInventoryDate==null){
			...
			btnImpInventoryDate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(ecsCheckStockAction.isExistsBySection(req, getSection(),ECSSemiFinishedExg.class)){						
						...
						ecsCheckStockAction.deleteECSSemiFinishedExg(req, getSection());
					...
					dipdie.setClazz("FmECSSemiFinishedExg");
...
//					报关商品名称；报关商品规格；报关单位；这算报关数量；折算系数；料件备案序号；
	...
	private void doFindStars() {
		...
		new AsynSwingWorker<List<?>>() {
			...
			protected List<?> submit() {
				...
				try{
					if(tableIndex == 0)
						return  ecsCheckStockAction.findECSSemiFinishedExg(request, getSection(), null);
					return    ecsCheckStockAction.findECSSemiFinishedExgResolve(request, getSection(), 
  ...
  //半成品折算成BOM
	public class resolveECSSemiFinishedExg extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {
			...
			disableControls();
			//半成品开始折料件BOM
			List<ECSSemiFinishedExgResolve> ecsSemiFinishedExgResolveLs=ecsCheckStockAction.
					resolveECSSemiFinishedExgResolves(request,getSection());
			return ecsSemiFinishedExgResolveLs;
		}
		@Override
		protected void done() {
			List<ECSSemiFinishedExgResolve>  semiFinishiedExgLs;
			try {
				semiFinishiedExgLs = (List<ECSSemiFinishedExgResolve>) get();
				tabbedPane.setSelectedIndex(1);
				initConvertJtable(semiFinishiedExgLs);
			} catch (InterruptedException e) {
				CommonProgress.closeProgressDialog();
				JOptionPane.showMessageDialog(FmECSSemiFinishedExg.this,e.getMessage());
	...
	private void doConvertStars() {
		new SwingWorker<List, Void>() {
			...
			protected List doInBackground() throws Exception {
				...
				try{
					return ecsCheckStockAction.convertECSSemiFinishedExgs(
							request, getSection());
				}finally{
			...
			@Override
			protected void done() {
				List<ECSSemiFinishedExgResolve>  semiFinishedExgLs;
				List errorInfos = new ArrayList<ErrorMessage>();
				try {
					semiFinishedExgLs = (List<ECSSemiFinishedExgResolve>) get();
					tabbedPane.setSelectedIndex(1);
					if(semiFinishedExgLs.size()>0){
						initConvertJtable(semiFinishedExgLs);
					}
					for (int i = 0; i < semiFinishedExgLs.size(); i++) {
						//如果返回的数据存在备案序号为空的代表这部分 
						//料件料号不存在归并关系，或者归并关系内容不完整需提醒
						ECSSemiFinishedExgResolve ecs = semiFinishedExgLs.get(i);
