 ...
 */
public class DgMaterielInner extends JDialogBase {
	...
	private InnerMergeData inner = null; //归并
        ...
        if(b){
            if (isAdd == false) {
                if (tableModel.getCurrentRow() != null) {
                    inner = (InnerMergeData) tableModel.getCurrentRow(); //归并
                    materiel = inner.getMateriel();//物料
                    scmCoi = materiel.getScmCoi(); //类别
                    complex = inner.getHsAfterComplex(); //商品编码
                }
                initUI();
                fillWindow();
                setState();
    ...
    private void initUI(){
        ...
        //报关单位
		this.jComboBox4.setModel(new DefaultComboBoxModel(CustomBaseList.getInstance().getUnits().toArray()));
		this.jComboBox4.setRenderer(CustomBaseRender.getInstance().getRender(
						"code", "name"));
		CustomBaseComboBoxEditor.getInstance().setComboBoxEditor(
						this.jComboBox4, "code", "name");	
		jComboBox4.setSelectedIndex(-1);
		//供应商
		List listscm = materialManageAction.findScmManufacturer(new Request(CommonVars
				.getCurrUser()));
	...
	private void fillWindow() {
		if (materiel != null && isAdd == false) {						
			...
			this.jTextField1.setText(inner.getHsAfterMaterielTenName()); //报关商品名称
			this.jTextField2.setText(inner.getHsAfterMaterielTenSpec()); //报关商品规格
			tfptDeSpec.setText(materiel.getPtDeSpec());     //工厂详细规格
			tfptPrice.setText(doubleToStr(materiel.getPtPrice()));         //单价
			jFormattedTextField4.setText(doubleToStr(materiel.getAmount()));//库存数量
			tfptNetWeight.setText(doubleToStr(materiel.getPtNetWeight())); //净重
			tfptOutWeight.setText(doubleToStr(materiel.getPtOutWeight())); //毛重
			jFormattedTextField2.setText(doubleToStr(materiel.getCknetWeight())); //仓库净重
			jFormattedTextField3.setText(doubleToStr(materiel.getCkoutWeight())); //仓库毛重
			jFormattedTextField1.setText(doubleToStr(materiel.getScrapWeight()));//废料重量
			...
			DgMaterielInner.this.jTextField.setText(materiel.getEqualPtno());
			this.cftaxation.setSelectedItem(materiel.getTaxation());
			this.jFormattedTextField.setText(doubleToStr(materiel.getUnitConvert()));
			...
			if (this.materiel.getCalUnit() != null) //工厂单位
			{
				this.jComboBox1.setSelectedItem(materiel.getCalUnit());
			} else {
				this.jComboBox1.setSelectedItem(null);
			}
			if (this.materiel.getScrapCode() != null){ //废料代码
				this.jComboBox5.setSelectedItem(materiel.getScrapCode());
			} else {
				this.jComboBox5.setSelectedItem(null);
			}
			if (this.materiel.getMaterialSource() != null){ //材料来源
				this.jComboBox6.setSelectedItem(materiel.getMaterialSource());
			} else {
				this.jComboBox6.setSelectedItem(null);
			}
			if (this.materiel.getCalWeightUnit() != null) //重量单位
			{
				this.jComboBox2.setSelectedItem(materiel.getCalWeightUnit());
			} else {
				this.jComboBox2.setSelectedItem(null);
			}
			if (this.inner.getHsAfterMemoUnit() != null) //报关单位
			{
				this.jComboBox4.setSelectedItem(inner.getHsAfterMemoUnit());
			} else {
			...
			{
				this.jComboBox7.setSelectedItem(materiel.getWrap());
			} else {
			...
			{
				this.jComboBox3.setSelectedItem(materiel.getScmCoc());
			} else {
	...
	private JTextField getTfptNo() {
		if (tfptNo == null) {
			...
			tfptNo.setBounds(261, 15, 106, 22);
		}
		return tfptNo;
	}
	...
	private JTextField getTfcomplex() {
		if (tfcomplex == null) {
			...
			tfcomplex.setBounds(464, 15, 80, 22);
		}
		return tfcomplex;
	}
	...
	private JButton getJButton1() {
		if (jButton1 == null) {
			...
			jButton1.setText("...");
			jButton1.setBounds(543, 15, 24, 21);
			jButton1.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (complex != null) {
						DgMaterielInner.this.complex = complex;
						getTfcomplex().setText(complex.getCode());
						DgMaterielInner.this.getJTextField1().setText(complex.getName());
	...
	private JTextField getTfptName() {
		if (tfptName == null) {
			...
			tfptName.setBounds(69, 45, 117, 22);
		}
		return tfptName;
	}
	...
	private JTextField getTfptSpec() {
		if (tfptSpec == null) {
			...
			tfptSpec.setBounds(261, 45, 106, 22);
		}
		return tfptSpec;
	}
	...
	private JTextField getTfptDeSpec() {
		if (tfptDeSpec == null) {
			...
			tfptDeSpec.setBounds(464, 45, 102, 22);
		}
		return tfptDeSpec;
	}
	...
	private JFormattedTextField getTfptPrice() {
		if (tfptPrice == null) {
			...
			tfptPrice.setBounds(262, 73, 106, 22);
		}
		return tfptPrice;
	}
	...
	private JFormattedTextField getTfptNetWeight() {
		if (tfptNetWeight == null) {
			...
			tfptNetWeight.setBounds(70, 108, 115, 22);
		}
	...
	private JFormattedTextField getTfptOutWeight() {
		if (tfptOutWeight == null) {
			...
			tfptOutWeight.setBounds(261, 105, 105, 22);
		}
	...
	private JTextField getTfptVersion() {
		if (tfptVersion == null) {
			...
			tfptVersion.setBounds(464, 137, 84, 22);
		}
		return tfptVersion;
	}
	...
	private JTextField getTfptEnglishName() {
		if (tfptEnglishName == null) {
			...
			tfptEnglishName.setBounds(69, 137, 117, 22);
		}
	...
	private JTextField getTfptEnglishSpec() {
		if (tfptEnglishSpec == null) {
			...
			tfptEnglishSpec.setBounds(260, 137, 106, 22);
		}
	...
	private JComboBox getCfsysArea() {
		if (cfsysArea == null) {
			...
			cfsysArea.setBounds(69, 167, 117, 21);
		}
		return cfsysArea;
	}
	...
	private JComboBox getCftaxation() {
		if (cftaxation == null) {
			...
			cftaxation.setBounds(260, 167, 106, 22);
		}
		return cftaxation;
	}
	...
	private JComboBox getCfproBonded() {
		if (cfproBonded == null) {
			...
			cfproBonded.setBounds(464, 167, 102, 21);
		}
		return cfproBonded;
	}
	...
	private boolean checkNull() { //检查是否为空
		...
		if (this.jComboBox2.getSelectedItem()==null) {
			JOptionPane.showMessageDialog(this, "重量单位不能为空！", "提示！", 0);
			return true;
		}
		if (this.tfptNetWeight.getText().trim().equals("")) {
			JOptionPane.showMessageDialog(this, "净重不能为空！", "提示！", 0);
			return true;
		}
		return false;
	}

	public void fillData() { //保存
		...
		materiel.setUnitConvert(strToDouble(this.jFormattedTextField.getText()));//单位系数
		...
		if (cfsysArea.getSelectedItem() != null && !cfsysArea.getSelectedItem().equals("")){
		    materiel.setSysArea((SysArea) cfsysArea.getSelectedItem());//原产国
		}
		if (cftaxation.getSelectedItem() != null && !cftaxation.getSelectedItem().equals("")){
		...
		if (jComboBox7.getSelectedItem() != null){ //包装种类
	...
	private boolean checkMultiple() {
		Materiel ptNo = materialManageAction.findMaterielByPtNo(new Request(
				CommonVars.getCurrUser()), this.tfptNo.getText()
				.trim());
		if (ptNo != null) {
			if (!isAdd) {
	...
	private JButton getJButton5() {
		if (jButton5 == null) {
			...
			jButton5.setText("确定");
			jButton5.setBounds(210, 372, 74, 24);
			jButton5.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					fillData();
					inner.setUpdateDate(new Date());
					inner = commonBaseCodeAction.saveInner(new Request(CommonVars.getCurrUser()),inner,materiel);
					tableModel.updateRow(inner);
					DgMaterielInner.this.dispose();
				}
			});
	...
	private JButton getJButton6() {
		if (jButton6 == null) {
			...
			jButton6.setText("取消");
			jButton6.setBounds(309, 372, 75, 24);
	...
	private JTextField getJTextField() {
		if (jTextField == null) {
			...
			jTextField.setBounds(259, 258, 108, 22);
		}
		return jTextField;
	}
  /* private void getEqualPtNo(){
		DgMaterielInner.this.jTextField.setVisible(((ScmCoi) jComboBox.getSelectedItem()).getName().equals("边角料"));
		DgMaterielInner.this.jLabel14.setVisible(((ScmCoi) jComboBox.getSelectedItem()).getName().equals("边角料"));
   }*/
	/**
	...
	private JPanel getJPanel1() {
		if (jPanel1 == null) {
			...
			jLabel.setText("类别");
			jLabel.setBounds(14, 15, 30, 19);
			jLabel.setForeground(new java.awt.Color(51,51,255));
			jLabel1.setText("料号");
			jLabel1.setBounds(203, 15, 30, 20);
			jLabel1.setForeground(java.awt.Color.blue);
			jLabel2.setText("商品编码");
			jLabel2.setBounds(386, 15, 56, 21);
			jLabel3.setText("商品名称");
			jLabel3.setBounds(14, 45, 50, 22);
			jLabel3.setForeground(java.awt.Color.blue);
			jLabel4.setText("型号规格");
			jLabel4.setBounds(203, 45, 50, 22);
			jLabel5.setText("详细型号规格");
			jLabel5.setBounds(386, 45, 77, 21);
			jLabel6.setText("单位");
			jLabel6.setBounds(15, 73, 44, 21);
			jLabel6.setForeground(java.awt.Color.blue);
			jLabel7.setText("单价");
			jLabel7.setBounds(204, 73, 42, 20);
			jLabel8.setText("净重");
			jLabel8.setBounds(15, 106, 50, 20);
			jLabel8.setForeground(java.awt.Color.blue);
			jLabel10.setText("毛重");
			jLabel10.setBounds(203, 105, 49, 22);
			jLabel11.setText("版本号");
			jLabel11.setBounds(386, 137, 45, 22);
			jLabel12.setText("英文品名");
			jLabel12.setBounds(15, 137, 51, 23);
			jLabel13.setText("英文规格");
			jLabel13.setBounds(203, 137, 54, 23);
			jLabel15.setText("供应商");
			jLabel15.setBounds(386, 73, 41, 22);
			jLabel16.setText("原产国");
			jLabel16.setBounds(15, 165, 44, 22);
			jLabel17.setText("税制代码");
			jLabel17.setBounds(203, 167, 51, 21);
			jLabel18.setText("保纳税");
			jLabel18.setBounds(386, 167, 43, 19);
			jLabel9.setText("重量单位");
			jLabel9.setBounds(386, 105, 55, 22);
			jLabel9.setForeground(java.awt.Color.blue);
			jLabel22.setBounds(189, 197, 76, 21);
			jLabel22.setText("报关商品名称");
			jLabel23.setBounds(386, 196, 75, 21);
			jLabel23.setText("报关商品规格");
			jLabel24.setBounds(14, 326, 174, 18);
			jLabel24.setText("* 注：1,标记为蓝色字体为必填项");
			jLabel24.setForeground(new java.awt.Color(153,51,0));
			jLabel25.setBounds(190, 226, 54, 22);
			jLabel25.setText("报关单位");
			jLabel26.setBounds(386, 225, 77, 22);
			jLabel26.setText("单位折算系数");
			jLabel27.setBounds(15, 195, 52, 20);
			jLabel27.setText("废料代码");
			jLabel28.setBounds(16, 224, 57, 20);
			jLabel28.setText("废料重量");
			jLabel29.setBounds(16, 256, 53, 20);
			jLabel29.setText("材料来源");
			jLabel30.setBounds(46, 345, 189, 18);
			...
			jLabel36.setBounds(193, 286, 52, 23);
			jLabel36.setText("仓库净重");
			jLabel37.setBounds(385, 286, 60, 22);
			jLabel37.setText("仓库毛重");
			jLabel38.setBounds(386, 259, 59, 18);
	...
	private JPanel getJPanel2() {
		if (jPanel2 == null) {
			...
			jLabel19.setText("");
			jLabel19.setIcon(new ImageIcon(getClass().getResource("/com/bestway/bcus/client/resources/images/titlepic.jpg")));
			jLabel20.setText("物料主档");
			jLabel20.setFont(new java.awt.Font("Dialog", java.awt.Font.BOLD, 20));
			jLabel20.setForeground(new java.awt.Color(255,153,0));
			jLabel21.setText("");
			jLabel21.setIcon(new ImageIcon(getClass().getResource("/com/bestway/bcus/client/resources/images/titlepoint.jpg")));
	...
	/**
	 * @param scmCoi The scmCoi to set.
	 */
	...
	private JTextField getJTextField1() {
		if (jTextField1 == null) {
			...
			jTextField1.setBounds(264, 196, 103, 22);
		}
		return jTextField1;
	}
	/**
	...
	private JTextField getJTextField2() {
		if (jTextField2 == null) {
			...
			jTextField2.setBounds(463, 196, 104, 22);
		}
		return jTextField2;
	}
	/**
	...
	private JComboBox getJComboBox() {
		if (jComboBox == null) {
			...
			jComboBox.setBounds(69, 14, 117, 21);
		}
		return jComboBox;
	}
	/**
	...
	private JComboBox getJComboBox1() {
		if (jComboBox1 == null) {
			...
			jComboBox1.setBounds(70, 73, 115, 23);
		}
		return jComboBox1;
	}
	/**
	...
	private JComboBox getJComboBox2() {
		if (jComboBox2 == null) {
			...
			jComboBox2.setBounds(463, 105, 104, 23);
		}
		return jComboBox2;
	}
	/**
	...
	private JComboBox getJComboBox3() {
		if (jComboBox3 == null) {
			...
			jComboBox3.setBounds(464, 72, 103, 23);
		}
		return jComboBox3;
	}
	/**
	...
	private void setState(){
		List dateList=tableModel.getCurrentRows();
		List list=commonBaseCodeAction.findInnerMergeInEmsEdiMergeBefore(CommonUtils.getRequest(),dateList);
	...
	private JComboBox getJComboBox4() {
		if (jComboBox4 == null) {
			...
			jComboBox4.setBounds(258, 226, 108, 22);
		}
		return jComboBox4;
	}
	/**
	...
	private JFormattedTextField getJFormattedTextField() {
		if (jFormattedTextField == null) {
			...
			jFormattedTextField.setBounds(463, 225, 104, 22);
		}
	...
	private JComboBox getJComboBox5() {
		if (jComboBox5 == null) {
			...
			jComboBox5.setBounds(70, 195, 115, 20);
		}
		return jComboBox5;
	}
	/**
	...
	private JFormattedTextField getJFormattedTextField1() {
		if (jFormattedTextField1 == null) {
			...
			jFormattedTextField1.setBounds(70, 224, 117, 22);
		}
	...
	private JComboBox getJComboBox6() {
		if (jComboBox6 == null) {
			jComboBox6 = new JComboBox(new String[]{"自制","国内购买","国外采购","委外加工","客户供料"});
			jComboBox6.setBounds(71, 256, 115, 22);
		}
		return jComboBox6;
	}
	/**
	...
	private JComboBox getJComboBox7() {
		if (jComboBox7 == null) {
			...
			jComboBox7.setBounds(71, 286, 115, 22);
		}
		return jComboBox7;
	}
	/**
	...
	private JFormattedTextField getJFormattedTextField2() {
		if (jFormattedTextField2 == null) {
			...
			jFormattedTextField2.setBounds(259, 286, 106, 22);
		}
	...
	private JFormattedTextField getJFormattedTextField3() {
		if (jFormattedTextField3 == null) {
			...
			jFormattedTextField3.setBounds(461, 286, 104, 22);
		}
	...
	private JFormattedTextField getJFormattedTextField4() {
		if (jFormattedTextField4 == null) {
			...
			jFormattedTextField4.setBounds(462, 256, 103, 22);
		}
	...
	private JButton getJButton() {
		if (jButton == null) {
			jButton = new JButton();
			jButton.setBounds(new java.awt.Rectangle(546,136,20,23));
			jButton.setText("...");
			jButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					MaterialBomVersion version = (MaterialBomVersion) CommonQuery.getInstance().getVersionFromBgBom(tfptNo.getText());
			        tfptVersion.setText(version == null?null:String.valueOf(version.getVersion()));
				}
			});
		}
		return jButton;
