...
public class DgFactoryCheckImg extends JDialogBase {
	...
	public DgFactoryCheckImg(JTableListModel tableModel) {
		this.setTitle("工厂盘点料件");
	...
	private JButton getBtnSave() {
		if (btnSave == null) {
			...
			btnSave.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					FactoryCheckImg img = (FactoryCheckImg)tableModel.getCurrentRow();
					if(tfBgNum.getValue()!=null){
						img.setBgNum(Double.valueOf(tfBgNum.getValue().toString()));
					}
					
					if(tfMaterStockNum.getValue()!=null){
						img.setMaterStockNum(Double.valueOf(tfMaterStockNum.getValue().toString()));
					}
					img = checkCancelAction.saveFactoryCheckImg(new Request(CommonVars
							.getCurrUser()), img);
	...
	private JButton getBtnUp() {
		if (btnUp == null) {
			...
			btnUp.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					FactoryCheckImg img = (FactoryCheckImg)tableModel
							.getDataByRow(tableModel.getTable().getSelectedRow()-1);
					dataState = DataState.EDIT;
					tableModel.setSelectRow(tableModel.getTable().getSelectedRow()-1);
					fillData(img);
					setState();
				}
			});
	...
	private void fillData(FactoryCheckImg img){
		if(img==null){
			return;
		}
		...
		tfCalUnitName.setText(img.getCalUnit()==null?"":img.getCalUnit().getName());
		tfMaterStockNum.setValue(img.getMaterStockNum());
		tfUnitName.setText(img.getUnit()==null?"":img.getUnit().getName());
