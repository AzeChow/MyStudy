 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class DgBillSort extends JDialogBase {
	...
	public void setVisible(boolean b) {
		if (b) {
			...
			for (int i=0;i<list.size();i++){
				AtcMergeBeforeComInfo obj = (AtcMergeBeforeComInfo) list.get(i);
				obj.setSerialNo(i+1);
				listS.add(obj);
			}
	...
	class UserListCellRenderer extends DefaultListCellRenderer {
		public Component getListCellRendererComponent(JList list, Object value,
			...
			String salesCountry="";
			if (value != null) {
				AtcMergeBeforeComInfo obj = ((AtcMergeBeforeComInfo) value);
				salesCountry = obj.getSalesCountry()==null?"":obj.getSalesCountry().getName();
				s = obj.getSerialNo()+"   //"+ obj.getAfterComInfo().getEmsSerialNo() +"   //"+ obj.getAfterComInfo().getComplex().getCode() +"   //"+ obj.getMateriel().getFactoryName() +"   //"+ obj.getMateriel().getFactorySpec()+"   //"+ salesCountry;			
			}
			setText(s);
			return this;
		}
	...
	private void move(boolean isup){
		if (jList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(DgBillSort.this, "请选择要移动的项!", "提示", 2);
			return;
		}
		...
		AtcMergeBeforeComInfo selectinfo = null;
		AtcMergeBeforeComInfo preinfo = null;
		if (isup){//向上移动
			selectinfo = (AtcMergeBeforeComInfo) selectObj;
			selectinfo.setSerialNo(selectinfo.getSerialNo()-1);		
			preinfo = (AtcMergeBeforeComInfo) jList.getModel().getElementAt(x-1);
			preinfo.setSerialNo(preinfo.getSerialNo()+1);
	...
	private JButton getJButton2() {
		if (jButton2 == null) {
			...
			jButton2.addActionListener(new java.awt.event.ActionListener() { 
				public void actionPerformed(java.awt.event.ActionEvent e) {    			
					Vector list = DgBillSort.this.getList();
					baseEncAction.saveAtcMergeBeforeComInfo(new Request(CommonVars.getCurrUser()),list);
					dispose();
				}
			});
		}
