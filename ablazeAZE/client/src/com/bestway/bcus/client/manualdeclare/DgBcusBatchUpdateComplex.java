 ...
 * 
 */
public class DgBcusBatchUpdateComplex extends JDialogBase {
	 ...
	 * 
	 */
	public DgBcusBatchUpdateComplex() {
		super();
		manualDecleareAction = (ManualDeclareAction) CommonVars
				.getApplicationContext().getBean("manualdeclearAction");
		manualDecleareAction.authUpdateComplex(new Request(CommonVars
				.getCurrUser()));
		initialize();
	}
					...
					if (step == 0) {
						if (getPnBatchUpdateComplex().checkSelectType()) {
							JOptionPane.showMessageDialog(
									DgBcusBatchUpdateComplex.this, "请选择修改类型",
									"提示！", JOptionPane.INFORMATION_MESSAGE);
							return;
						}
						if (getPnBatchUpdateComplex().checkSelectMateriel()) {
							JOptionPane.showMessageDialog(
									DgBcusBatchUpdateComplex.this, "请选择修改物料类型",
									"提示！", JOptionPane.INFORMATION_MESSAGE);
							return;
						}
					}
