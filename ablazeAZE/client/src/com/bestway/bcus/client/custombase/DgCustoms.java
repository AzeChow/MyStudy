 ...
 */
public class DgCustoms extends JDialogBase {
			...
			public void windowOpened(java.awt.event.WindowEvent e) {
				...
				if (isAdd == false) {
					if (tableModel.getCurrentRow() != null) {
						custom = (Customs) tableModel.getCurrentRow();
						scode = custom.getName();
					}
					fillWindow();
			...
			jButton.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (isAdd == true) {
						...
						fillCurrCode(custom);
						if (isReCode(DgCustoms.this.tfCode.getText())) {
							return;
						}
						if (isReName(DgCustoms.this.tfName.getText())) {
							return;
						}
						custom = customBaseAction.SaveCustom(custom);
						tableModel.addRow(custom);
						
						System.out.println("保存成功！");
					    clearData();
					} else {
						 fillCurrCode(custom);
						 if (isReName(custom.getName())){
						 return;
						 }
						 custom = customBaseAction.SaveCustom(custom);
						 Dialog();
						 tableModel.updateRow(custom);
						 DgCustoms.this.dispose();
					}
	...
	private boolean isReCode(String data) {
		if (customBaseAction.isReDgCustomCode(data)) {
	...
	private boolean isReName(String name) {
		if (customBaseAction.isReDgCustomName(name)) {
	...
	private void fillWindow() {
		...
		if (custom != null) {
			this.tfCode.setText(custom.getCode());
			this.tfName.setText(custom.getName());
		}
	}

	private void fillCurrCode(Customs custom) {
		custom.setCode(this.tfCode.getText().trim());
		custom.setName(this.tfName.getText().trim());
	}
