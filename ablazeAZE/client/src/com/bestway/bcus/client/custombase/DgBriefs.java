 ...
 */
public class DgBriefs extends JDialogBase {
			...
			public void windowOpened(java.awt.event.WindowEvent e) {
				...
				if (isAdd == false) {
					if (tableModel.getCurrentRow() != null) {
						brief = (Brief) tableModel.getCurrentRow();
						scode = brief.getName();
					}
					fillWindow();
			...
			jButton.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (isAdd == true) {
						...
						fillCurrCode(brief);
						if (isReCode(DgBriefs.this.tfCode.getText())) {
							return;
						}
						if (isReName(DgBriefs.this.tfName.getText())) {
							return;
						}
						brief = customBaseAction.SaveBrief(brief);
						tableModel.addRow(brief);
						
						System.out.println("保存成功！");
					    clearData();
					} else {
						 fillCurrCode(brief);
						 if (isReName(brief.getName())){
						 return;
						 }
						 brief = customBaseAction.SaveBrief(brief);
						 Dialog();
						 tableModel.updateRow(brief);
						 DgBriefs.this.dispose();
					}
	...
	private boolean isReCode(String data) {
		if (customBaseAction.isReDgBriefCode(data)) {
	...
	private boolean isReName(String name) {
		if (customBaseAction.isReDgBriefName(name)) {
	...
	private void fillWindow() {
		...
		if (brief != null) {
			this.tfCode.setText(brief.getCode());
			this.tfName.setText(brief.getName());
		}
	}

	private void fillCurrCode(Brief breif) {
		breif.setCode(this.tfCode.getText().trim());
		breif.setName(this.tfName.getText().trim());
	}
