 ...
 */
public class DgBlsFactoryCustoms extends JDialogBase {
	...
	public DgBlsFactoryCustoms() {
		...
		CustomFormattedTextFieldUtils.setFormatterFactory(this.jFormattedTextField, 5);
		CustomFormattedTextFieldUtils.setFormatterFactory(this.jFormattedTextField1, 5);
	}
	...
	public void setVisible(boolean isFlag) {
		if (tableModel.getCurrentRow() != null) {
			data = (BlsInnerMerge) tableModel.getCurrentRow();
		}
		showData();
		...
		if (btnCancel == null) {
			...
			btnCancel.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					DgBlsFactoryCustoms.this.dispose();
	...
	private void showData() {
		this.afterComplex = data.getBlsTenInnerMerge().getComplex();
		this.afterComUnit = data.getBlsTenInnerMerge().getComUnit();
		...
		jTextField.setText(data.getBlsTenInnerMerge().getComplex().getCode());
		jTextField3.setText(data.getBlsTenInnerMerge().getName());
		jTextField2
				.setText(data.getBlsTenInnerMerge().getComUnit() == null ? ""
						: data.getBlsTenInnerMerge().getComUnit().getName());
		jTextField4.setText(data.getBlsTenInnerMerge().getSpec() == null ? ""
				: data.getBlsTenInnerMerge().getSpec());
		this.jFormattedTextField.setValue(data.getUnitConvert());
		jFormattedTextField1.setValue(data.getMateriel().getPtNetWeight());
	}

	private BlsInnerMerge fillData() {
		BlsInnerMerge bim = (BlsInnerMerge) this.tableModel.getCurrentRow();
		...
		bim.getBlsTenInnerMerge().setName(jTextField3.getText().trim());
		bim.getBlsTenInnerMerge().setSpec(jTextField4.getText().trim());
		bim.getBlsTenInnerMerge().setComplex(this.afterComplex);
		bim.getBlsTenInnerMerge().setComUnit(this.afterComUnit);
		return bim;
	}
	...
	private JButton getJButton4() {
		if (btnOk == null) {
			...
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					BlsInnerMerge dim = fillData();
					Materiel materiel = dim.getMateriel();
					materiel = materialManageAction.saveMateriel(new Request(
							CommonVars.getCurrUser()), materiel);
					BlsTenInnerMerge blsTenInnerMerge = dim
							.getBlsTenInnerMerge();
					blsInnerMergeAction.saveBlsTenInnerMerge(new Request(
							CommonVars.getCurrUser()), blsTenInnerMerge);
					blsInnerMergeAction.saveBlsInnerMerge(new Request(
							CommonVars.getCurrUser()), dim);
					DgBlsFactoryCustoms.this.tableModel.updateRow(dim);
					DgBlsFactoryCustoms.this.dispose();
				}
			});
		}
