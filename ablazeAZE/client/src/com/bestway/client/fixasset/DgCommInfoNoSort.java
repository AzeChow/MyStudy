 ...
 */
public class DgCommInfoNoSort extends JDialogBase {
	...
	public void setVisible(boolean b) {
		if (b) {
			...
			for (int i = 0; i < list.size(); i++) {
				AgreementCommInfo obj = (AgreementCommInfo) list.get(i);
				obj.setMainNo(i + 1);
				listS.add(obj);
			}
	...
	class UserListCellRenderer extends DefaultListCellRenderer {
		public Component getListCellRendererComponent(JList list, Object value,
			...
			String s = "";
			if (value != null) {
				AgreementCommInfo obj = ((AgreementCommInfo) value);
				s = formatStringByLength(obj.getMainNo().toString(), 5)
	...
	private void move(boolean isup) {
		if (jList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(DgCommInfoNoSort.this, "请选择要移动的项!",
					"提示", 2);
			return;
		}
		...
		AgreementCommInfo selectinfo = null;
		AgreementCommInfo preinfo = null;
		if (isup) {// 向上移动
			selectinfo = (AgreementCommInfo) selectObj;
			selectinfo.setMainNo(selectinfo.getMainNo() - 1);
			preinfo = (AgreementCommInfo) jList.getModel().getElementAt(x - 1);
			preinfo.setMainNo(preinfo.getMainNo() + 1);
...
//			jList.sets
	...
	private JButton getBtnOk() {
		if (btnOk == null) {
			...
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Vector list = DgCommInfoNoSort.this.getList();
					fixAssetAction.saveAgreementCommInfo(new Request(CommonVars
							.getCurrUser()), list);
					isOk = true;
					dispose();
				}
	 ...
	 */
	public void setFixAssetAction(FixAssetAction baseEncAction) {
		this.fixAssetAction = baseEncAction;
	}
