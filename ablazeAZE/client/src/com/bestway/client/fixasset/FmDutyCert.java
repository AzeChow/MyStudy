...
public class FmDutyCert extends JInternalFrameBase {
	...
	private JCalendarComboBox cbbCertApproveDate = null;
	...
	private JTextField tfApplyCop = null;

	private JCalendarComboBox cbbCertEffectDate = null;
	...
	private void showHeadData() {
		...
		if (agreement != null) {
			list = fixAssetAction.findDutyCertHead(new Request(CommonVars
		...
		if (btnClose == null) {
			...
			btnClose.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					FmDutyCert.this.doDefaultCloseAction();
				}
			});
		}
		return btnClose;
			...
			cbbAgreement.addItemListener(new java.awt.event.ItemListener() {
				public void itemStateChanged(java.awt.event.ItemEvent e) {
					if (e.getStateChange() == java.awt.event.ItemEvent.SELECTED) {
						...
						// .getSelectedItem();
						// List list = fixAssetAction.findDutyCertHead(
						// new Request(CommonVars.getCurrUser()),
						// agreement.getSancEmsNo());
						// initHeadTable(list);
						showHeadData();
					}
				}
						...
						public void valueChanged(ListSelectionEvent e) {
							...
							if (!lsm.isSelectionEmpty()) {
								if (tableModel.getCurrentRow() != null) {
									DutyCertHead dutyCertHead = ((DutyCertHead) tableModel
											.getCurrentRow());
									list = fixAssetAction.findDutyCertDetail(
											new Request(CommonVars
													.getCurrUser()),
											dutyCertHead);
									showHeadData(dutyCertHead);
								}
							}
			...
			jTabbedPane
					.addChangeListener(new javax.swing.event.ChangeListener() {
						public void stateChanged(javax.swing.event.ChangeEvent e) {
							List list = new ArrayList();
							DutyCertHead dutyCertHead = ((DutyCertHead) tableModelHead
									.getCurrentRow());
	...
	private JPanel getJPanel2() {
		if (jPanel2 == null) {
			jLabel5 = new JLabel();
			jLabel5.setBounds(new Rectangle(34, 94, 59, 20));
			jLabel5.setText("有效日期");
			jLabel4 = new JLabel();
			jLabel4.setBounds(new Rectangle(34, 69, 59, 24));
			jLabel4.setText("发证日期");
			jLabel2 = new JLabel();
			jLabel2.setBounds(new Rectangle(34, 39, 59, 24));
			...
			jPanel2.add(getTfCertNo(), null);
			jPanel2.add(getCbbCertApproveDate(), null);
			jPanel2.add(getTfApplyCop(), null);
			jPanel2.add(getCbbCertEffectDate(), null);
			...
			tbDetail.getSelectionModel().addListSelectionListener(
					new ListSelectionListener() {
						public void valueChanged(ListSelectionEvent e) {
							...
							if (!lsm.isSelectionEmpty()) {
								if (tableModel.getCurrentRow() != null) {
									DutyCertDetail cutyCertDetail = ((DutyCertDetail) tableModel
											.getCurrentRow());
									showDetailData(cutyCertDetail);
								}
							}
	...
	private JTextField getTfCertNo() {
		if (tfCertNo == null) {
			...
			tfCertNo.setBounds(new java.awt.Rectangle(96, 12, 127, 25));
		}
		return tfCertNo;
	}
	 ...
	 */
	private JCalendarComboBox getCbbCertApproveDate() {
		if (cbbCertApproveDate == null) {
			cbbCertApproveDate = new JCalendarComboBox();
			cbbCertApproveDate.setBounds(new Rectangle(96, 67, 127, 22));
		}
		return cbbCertApproveDate;
	}
	...
	private void initHeadTable(List list) {
		JTableListModel.dataBind(tbHead, list, new JTableListModelAdapter() {
			public List InitColumns() {
				...
				list.add(addColumn("编号", "certNo", 100));
				list.add(addColumn("申请企业", "company.buName", 100));
				list.add(addColumn("申请单位编码", "company.buCode", 100));
				list.add(addColumn("发证日期", "certApproveDate", 100));
				list.add(addColumn("有效日期", "certEffectDate", 100));
				return list;
			}
		});
	...
	private void fillHeadData(DutyCertHead invoiceHead) {
		if (invoiceHead == null) {
			return;
		}
		invoiceHead.setCertNo(this.tfCertNo.getText().trim());
		invoiceHead.setCertApproveDate(this.cbbCertApproveDate.getDate());
		invoiceHead.setCertEffectDate(this.cbbCertEffectDate.getDate());
	}

	private void showHeadData(DutyCertHead dutyCertHead) {
		if (dutyCertHead == null) {
			return;
		}
		this.tfCertNo.setText(dutyCertHead.getCertNo());
		this.cbbCertApproveDate.setDate(dutyCertHead.getCertApproveDate());
		this.cbbCertEffectDate.setDate(dutyCertHead.getCertEffectDate());
		Company company = (Company) dutyCertHead.getCompany();
		this.tfApplyCop.setText(company.getBuName());
	}

	private void showDetailData(DutyCertDetail dutyCertDetail) {
		if (dutyCertDetail == null) {
			return;
		}
		this.tfCommName.setText(dutyCertDetail.getCommName());
		this.tfAmount.setValue(dutyCertDetail.getAmount());
		this.tfUnitPrice.setValue(dutyCertDetail.getUnitPrice());
		this.tfTotalPrice.setValue(dutyCertDetail.getTotalPrice());
	 ...
	 * @return javax.swing.JTextField
	 */
	private JTextField getTfApplyCop() {
		if (tfApplyCop == null) {
			tfApplyCop = new JTextField();
			tfApplyCop.setBounds(new Rectangle(96, 38, 127, 24));
			tfApplyCop.setEditable(false);
		}
		return tfApplyCop;
	}
	 ...
	 * @return com.bestway.ui.winuicontrol.calendar.JCalendarComboBox
	 */
	private JCalendarComboBox getCbbCertEffectDate() {
		if (cbbCertEffectDate == null) {
			cbbCertEffectDate = new JCalendarComboBox();
			cbbCertEffectDate.setBounds(new Rectangle(96, 93, 127, 21));
		}
		return cbbCertEffectDate;
	}
	...
	private JButton getJButton() {
		if (jButton == null) {
			jButton = new JButton();
			jButton.setBounds(new Rectangle(255, 13, 70, 24));
			jButton.setText("保存");
			jButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					checkFixAuthorityAction
							.checkSaveAgreementDutyCert(new Request(CommonVars
									.getCurrUser()));
					DutyCertHead dutyCertHead = ((DutyCertHead) tableModelHead
							.getCurrentRow());
					Agreement agreement = (Agreement) cbbAgreement
							.getSelectedItem();
					if (dutyCertHead == null || agreement == null) {
						return;
					}
					fillHeadData(dutyCertHead);
					dutyCertHead = fixAssetAction.saveDutyCertHead(new Request(
							CommonVars.getCurrUser()), agreement, dutyCertHead);
					List list = fixAssetAction.findDutyCertHead(new Request(CommonVars
								.getCurrUser()), agreement.getSancEmsNo());
					initHeadTable(list);					
				}
			});
	...
	private JButton getJButton1() {
		if (jButton1 == null) {
			...
			jButton1.setBounds(new Rectangle(255, 57, 70, 24));
			jButton1.setText("取消");
