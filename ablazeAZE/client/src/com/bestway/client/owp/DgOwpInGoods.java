...
public class DgOwpInGoods extends JDialogBase {
	/**
	 * 
	 */
	...
	private JPanel getPnTop1() {
		if (PnTop1 == null) {
			...
			lbEndTime.setBounds(new Rectangle(288, 49, 33, 22));
			lbEndTime.setText("至");
			...
			lbInTradeName.setBounds(new Rectangle(251, 23, 92, 22));
	...
	public void queryData(){
		if(tfOwpNo.getText()==null||tfOwpNo.getText().trim().equals("")){
			JOptionPane.showMessageDialog(DgOwpInGoods.this, "申请表编号不能为空", "提示", 0);
			return;
		}
		// 查询
		// 组织查询条件
	...
	class Find extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {// 后台计算
			...
			List list = null;
			// 查询
			CommonProgress.showProgressDialog(DgOwpInGoods.this);
			CommonProgress.setMessage("系统正在执行查询，请稍后...");
			String orderBy = "";
			list=owpAnalyAction.findOwpInGoods(conditions);
			return list;
		}
		...
		protected void done() {// 更新视图
			...
			for(int i=0;i<list.size();i++){
				System.out.println("list.trNo="+((OwpBillRecvItem)list.get(i)).getTrNo());
			}
			initTable(list);
		}
	}
	...
	private JButton getBtprint() {
		if (btnprint == null) {
			...
			btnprint.setLocation(new Point(645, 23));
	...
	private void printReport() {
		...
		try {
			...
			if (list == null || list.isEmpty()) {
				JOptionPane.showMessageDialog(DgOwpInGoods.this, "没有列印的记录！",
	...
	private JTextField getTfIntradeName() {
		if (tfIntradeName == null) {
			...
			tfIntradeName.setBounds(new Rectangle(360, 24, 172, 22));
	...
	private JCalendarComboBox getCbEndDate() {
		if (cbEndDate == null) {
			...
			cbEndDate.setBounds(new Rectangle(355, 49, 170, 22));
		}
		return cbEndDate;
	}
	...
	private JButton getBtnOwpNo() {
		if (btnOwpNo == null) {
			...
			btnOwpNo.setBounds(new Rectangle(220, 23, 18, 22));
