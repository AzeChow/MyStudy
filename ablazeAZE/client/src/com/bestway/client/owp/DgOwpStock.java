 ...
 *
 */
public class DgOwpStock extends JDialogBase {
	/**
	 * 顶部面板
	 */
	...
	private void initialize() {
		...
		this.setTitle("外发加工库存汇总表（按工厂单据统计）");
	...
	private JPanel getPnTop1() {
		if (PnTop1 == null) {
			...
			PnTop1.setBounds(new Rectangle(10, 10, 849, 111));
	...
	public void queryData(){
		...
		if(tfOwpNo.getText()==null||tfOwpNo.getText().trim().equals("")){
			JOptionPane.showMessageDialog(DgOwpStock.this, "申请表编号不能为空", "提示", 0);
			return;
		}
		// 查询
		// 组织查询条件
	...
	class Find extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {// 后台计算
			...
			List list = null;
			// 查询
			CommonProgress.showProgressDialog(DgOwpStock.this);
			CommonProgress.setMessage("系统正在执行查询，请稍后...");
			String orderBy = "";
			list=owpAnalyAction.findOwpStockAll(conditions);
			for(int i=0;i<list.size();i++){
				TempOwpStockAll temp=(TempOwpStockAll)list.get(i);
				System.out.println("实际返回边角料数量="+temp.getSjRemianAmount());
			}
			return list;
		}
	...
	private void printReport() {
		...
		try {
			...
			if (list == null || list.isEmpty()) {
				JOptionPane.showMessageDialog(DgOwpStock.this, "没有列印的记录！",
		...
		tableModel = new JTableListModel(tbTable, list,
				new JTableListModelAdapter(maximumFractionDigits) {
					public List<JTableListColumn> InitColumns() {
						...
						list.add(addColumn("外发序号", "listNo", 100));
						list.add(addColumn("商品名称", "hsName", 70));
						list.add(addColumn("商品规格", "hsSpec", 100));
						list.add(addColumn("计量单位", "hsUnit.name", 100));
						list.add(addColumn("累计发货数量", "outAllAmount", 100));
						list.add(addColumn("累计退回数量", "returnAllAmount", 100));
						list.add(addColumn("总发货数量", "allAmount", 100));
						
						list.add(addColumn("返回折料数量", "returnBomAmount", 100));
						list.add(addColumn("返回残次品折料数量", "returnBadBomAmount", 120));
						list.add(addColumn("应返回边角料数量", "yinRemianAmount", 120));

						list.add(addColumn("实际返回边角料数量", "sjRemianAmount", 120));
						list.add(addColumn("外发加工库存", "owpStockAmount", 100));
						list.add(addColumn(" 外发加工边角料库存", "owpRemianAmout", 120));
	...
	private JTextField getTfIntradeName() {
		if (tfIntradeName == null) {
			...
			tfIntradeName.setBounds(new Rectangle(356, 24, 172, 22));
	...
	private JCalendarComboBox getCbStartDate() {
		if (cbStartDate == null) {
			...
			cbStartDate.setBounds(new Rectangle(82, 52, 169, 22));
		}
		return cbStartDate;
	}
	...
	private JCalendarComboBox getCbEndDate() {
		if (cbEndDate == null) {
			...
			cbEndDate.setBounds(new Rectangle(356, 52, 170, 22));
		}
		return cbEndDate;
	}
	...
	private JTextField getTfPtNo() {
		if (tfPtNo == null) {
			...
			tfPtNo.setBounds(new Rectangle(79, 82, 151, 22));
		}
		return tfPtNo;
	}
	...
	private JButton getBtnPtNo() {
		if (btnPtNo == null) {
			btnPtNo = new JButton();
			btnPtNo.setBounds(new Rectangle(232, 82, 18, 22));
			btnPtNo.setText("…");
		}
		return btnPtNo;
	}
	...
	private JButton getBtnOwpNo() {
		if (btnOwpNo == null) {
			...
			btnOwpNo.setBounds(new Rectangle(232, 22, 18, 22));
			btnOwpNo.setText("…");
	...
	private JTextField getTfHsName() {
		if (tfHsName == null) {
			...
			tfHsName.setBounds(new Rectangle(356, 82, 172, 22));
