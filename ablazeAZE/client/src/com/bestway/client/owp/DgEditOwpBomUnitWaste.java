...
public class DgEditOwpBomUnitWaste extends JDialogBase {
	...
	private OwpAnalyAction owpAnalyAction = null; // @jve:decl-index=0:
		...
		if (pnCommonQueryPage == null) {
			pnCommonQueryPage = new PnCommonQueryPage() {
				...
				public JTableListModel initTable(List dataSource) {
					return DgEditOwpBomUnitWaste.this
				...
				public List getDataSource(int index, int length,
					...
					return DgEditOwpBomUnitWaste.this.getDataSource(index,
		...
		tableModelMaterial = new JTableListModel(tbMaterial, list,
				new JTableListModelAdapter() {
					public List InitColumns() {
						...
						list.add(addColumn("料号", "compentNo", 100));
						list.add(addColumn("工厂商品名称", "compentName", 100));
						list.add(addColumn("工厂型号规格", "compentSpec", 100));
						return list;
					}
				});
		this.showUnitWaste();
	...
	public List getDataSource(int index, int length, String property,
		...
		if (owpAnalyAction == null) {
			return new ArrayList();
		}
		List list= owpAnalyAction.findOwpBomMateriel(new Request(
				CommonVars.getCurrUser()), index, length, property, value,
				isLike);
		Map map=new HashMap<String, OwpBomDetail>();
		for(int i=0;i<list.size();i++){
			OwpBomDetail manage=(OwpBomDetail)list.get(i);
			if(map.get(manage.getCompentNo())==null)
				map.put(manage.getCompentNo(),manage);
		}
	...
	private JTableListModel initUnitWasteTable(List list) {
		JTableListModelAdapter jTableListModelAdapter = new JTableListModelAdapter() {
			public List InitColumns() {
				List<JTableListColumn> list = new Vector<JTableListColumn>();
				list.add(addColumn("父料号", "version.parent.ptNo", 80));
				list.add(addColumn("BOM版本", "version.version", 100));
				list.add(addColumn("子料号", "compentNo", 100));
				list.add(addColumn("子料名称", "compentName", 70));
	...
	private void commitValueChange(final JFormattedTextField tf) {
		...
		OwpBomDetail detail = (OwpBomDetail) tableModelUnitWaste
				.getCurrentRow();
	...
	private void showUnitWaste() {
		...
		if (tableModelMaterial != null) {
			OwpBomDetail materiel = (OwpBomDetail) (tableModelMaterial.getCurrentRow());
			if (materiel != null) {
				list = owpAnalyAction.findMaterialBomByDetail(
					...
					if (tableModelUnitWaste != null) {
						...
						for (int i = 0; i < list.size(); i++) {
							OwpBomDetail detail = (OwpBomDetail) list
									.get(i);
							detail.setUnitUsed(detail.getUnitWare()/(1-detail.getWare()));
							owpAnalyAction.saveOwpBomManage(
