 ...
 *
 */
public class FmOwpBillSend extends JInternalFrameBase {
...
//				.getCurrUser()),appNo);
	...
	private void setState() {
		...
		OwpBillSendHead c = null;
		if(null!=tableModel){
			c = (OwpBillSendHead) tableModel.getCurrentRow();
		}
		if (c == null) {
			return;
		}
...
//					dg.setVisible(true);
			...
			btnEdit.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (tableModel.getCurrentRow() == null) {
						JOptionPane.showMessageDialog(FmOwpBillSend.this,
								"请选择你要修改的资料", "提示", 0);
						return;
					}
					DgOwpBillSend dg = new DgOwpBillSend();
	...
	private void deleteBillHead(){
		...
		if (JOptionPane.showConfirmDialog(FmOwpBillSend.this,
				"确定要删除吗？", "确认", 0) != 0) {
			return;
		}
...
//				CommonVars.getCurrUser()), head);
		owpBillAction.deleteOwpSendBillHead(new Request(
				CommonVars.getCurrUser()), head);
	...
	private JButton getBtnDeclare() {
		if (btnDeclare == null) {
			...
			btnDeclare.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					OwpBillSendHead owpBillSendHead = (OwpBillSendHead) tableModel.getCurrentRow();
					...
					owpBillSendHead = owpBillAction.findOwpBillSendHeadById(new Request(
							CommonVars.getCurrUser()), owpBillSendHead.getId());
					tableModel.updateRow(owpBillSendHead);
	...
	class DeclareThread extends Thread {
		public void run() {
			try {
				...
				OwpBillSendHead head = (OwpBillSendHead) tableModel.getCurrentRow();
				
				try {
					DeclareFileInfo fileInfo = owpBillAction
							.declareOwpBillSendHead(new Request(CommonVars
									.getCurrUser()), head, taskId);
					
					
					head = owpBillAction.findOwpBillSendHeadById(new Request(
							CommonVars.getCurrUser()), head.getId());
					if (head != null) {
						tableModel.updateRow(head);
					}
					CommonStepProgress.closeStepProgressDialog();
					JOptionPane.showMessageDialog(FmOwpBillSend.this, fileInfo
							.getFileInfoSpec(), "提示", 1);
				} catch (Exception ex) {
					CommonStepProgress.closeStepProgressDialog();
					JOptionPane.showMessageDialog(FmOwpBillSend.this, "系统申报失败 "
							+ ex.getMessage(), "确认", 1);
				}
//				setState();
			} catch (Exception ex) {
		...
		if (btnAnnul == null) {
			...
			btnAnnul.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					//判断登记表为最后一次审批通过
//					List list = owpBillAction.findOwpBillSendHead(new Request(
//							CommonVars.getCurrUser()),owpAppHead.getAppNo());
//					if(list!=null&&list.size()>0){
//						OwpBillSendHead owpBillSendHead = (OwpBillSendHead)list.get(list.size()-1);
//						OwpBillSendHead head = (OwpBillSendHead)tableModel.getCurrentRow();
//						if(!owpBillSendHead.getSeqNum().equals(head.getSeqNum())){
//							JOptionPane.showMessageDialog(FmOwpBillSend.this,
...
//					}
	...
	private void process() {
		if (tableModel.getCurrentRow() == null) {
			JOptionPane.showMessageDialog(FmOwpBillSend.this, "请选择要回执处理的合同",
					"提示", 2);
			return;
		}
		OwpBillSendHead head = (OwpBillSendHead) tableModel
...
//		head = this.owpBillAction.findOwpBillSendHeadById(new Request(
//					CommonVars.getCurrUser()), head.getId());
		
		System.out.println("获得变更前的存在的数据");
		// 获得变更前的存在的数据
		OwpBillSendHead existOwpBillSendHead = null;
		...
		List lsReturnFile = new ArrayList();
			...
			lsReturnFile = OwpMessageHelper.getInstance().showOwpReceiptFile(
					OwpBusinessType.OWP_BILL_SEND, copBillNo);
			if (lsReturnFile.size() <= 0) {
				return;
			}
			
			existOwpBillSendHead = this.owpBillAction.findOwpBillSendHeadByCopBillNo(
		...
		try {
			...
			String result = owpBillAction.processOwpBillSendHead(new Request(
					CommonVars.getCurrUser()), head, existOwpBillSendHead,
					lsReturnFile);
			...
			if (existOwpBillSendHead != null) {
				OwpBillSendHead exingHeadTemp = owpBillAction.findOwpBillSendHeadById(new Request(
						CommonVars.getCurrUser()), existOwpBillSendHead.getId());
				if (exingHeadTemp == null) {
					tableModel.deleteRow(existOwpBillSendHead);
				}
			}
			JOptionPane.showMessageDialog(FmOwpBillSend.this, "回执处理成功！\n"
					+ result, "提示", JOptionPane.INFORMATION_MESSAGE);
		} catch (Exception ex) {
			JOptionPane.showMessageDialog(FmOwpBillSend.this, "回执处理失败"
	...
	private JButton getBtnClose() {
		if (btnClose == null) {
			...
			btnClose.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					FmOwpBillSend.this.doDefaultCloseAction();
				}
			});
		}
		return btnClose;
