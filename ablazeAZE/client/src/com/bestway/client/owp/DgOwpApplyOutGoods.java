...
public class DgOwpApplyOutGoods extends JDialogBase {
	/**
	 * 
	 */
	...
	private JPanel getPnTop1() {
		if (PnTop1 == null) {
			...
			lbInTradeName.setBounds(new Rectangle(3, 55, 92, 22));
			...
			PnTop1.setBounds(new Rectangle(10, 10, 849, 96));
	...
	public void queryData(){
		if((tfOwpNo.getText()==null||tfOwpNo.getText().trim().equals(""))
			...
			JOptionPane.showMessageDialog(DgOwpApplyOutGoods.this, "申请表编号或承揽方企业名称至少填写一个", "提示", 0);
			return;
		}
		if(!cbState1.isSelected()&&!cbState2.isSelected()&&!cbState3.isSelected()
				&&!cbState4.isSelected()){
			JOptionPane.showMessageDialog(DgOwpApplyOutGoods.this, "请至少选择一种要查询的申报状态", "提示", 0);
			return;
		}
		// 查询
		// 组织查询条件
	...
	class Find extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {// 后台计算
			...
			List list = null;
			// 查询
			CommonProgress.showProgressDialog(DgOwpApplyOutGoods.this);
			CommonProgress.setMessage("系统正在执行查询，请稍后...");
			String orderBy = "";
			list=owpAnalyAction.findOwpApplyOutGoods(appConditions,billConditions);
			return list;
		}
	...
	private void printReport() {
		...
		try {
			...
			if (list == null || list.isEmpty()) {
				JOptionPane.showMessageDialog(DgOwpApplyOutGoods.this, "没有列印的记录！",
		...
		tableModel = new JTableListModel(tbTable, list,
				new JTableListModelAdapter(maximumFractionDigits) {
					public List<JTableListColumn> InitColumns() {
						...
						list.add(addColumn("累计发货数量", "countQty", 100));
						list.add(addColumn("可发货数量", "canQty", 100));
	...
	private JTextField getTfIntradeName() {
		if (tfIntradeName == null) {
			...
			tfIntradeName.setBounds(new Rectangle(100, 55, 151, 22));
			tfIntradeName.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
	...
	private JButton getBtnIntradeName() {
		if (btnIntradeName == null) {
			...
			btnIntradeName.setBounds(new Rectangle(257, 55, 18, 22));
