...
public class DgFromMaterielEdit extends JDialogBase {
	...
	private void initdata(int pagenum){		
		...
		if (materiel.getCalWeightUnit()!= null && materiel.getCalWeightUnit().getName() != null 
				&& materiel.getCalWeightUnit().getCode().equals("036")){//036代表‘克’，'公克'等。
			weightRate = 0.001;
		} else {
			weightRate = 1.0;
		}
	...
	private void getZj_CK(){
		...
		double materielStorNetght = materiel.getCknetWeight()==null?0:(materiel.getCknetWeight().doubleValue());
		jLabel22.setText("总净重-(仓库净重*申报数量)："+String.valueOf(totalNetweight-(materielStorNetght*declareNum)));
	}
	/**
			...
			jButton.addActionListener(new java.awt.event.ActionListener() { 
				public void actionPerformed(java.awt.event.ActionEvent e) { 
					...
					if ((materielNetweight - materielStorNetweight)>0.5){
						if (JOptionPane.showConfirmDialog(DgFromMaterielEdit.this,"净重比仓库净重大于0.5,是否继续?","提示",0)!=0){
							return;
						}
					}
					double totalNetweight = (jFormattedTextField.getText()==null || jFormattedTextField.getText().equals(""))?0:Double.parseDouble(jFormattedTextField.getText());
					...
					if (totalNetweight-(materielStorNetweight*declareNum)>300){
						if (JOptionPane.showConfirmDialog(DgFromMaterielEdit.this,"总净重-(仓库净重*申报数量)大于300,是否继续?","提示",0)!=0){
							return;
						}
					}
					...
					if (isBrowse){								
						...
						bcusEncAction.SaveCustomsFromMateriel(new Request(CommonVars.getCurrUser()),(CustomsFromMateriel)fromMateriel);						
							...
							if (bcusimgbill != null  || bcsimgbill != null){ 
								...
								double yshuliang =  Double.parseDouble(jFormattedTextField3.getText().toString());//报关数量=申报数量
								commInfo.setCommAmount(commAmount);//数量						
								if (firstUnit != null && firstUnit.getName().trim().equals("米")) {
									firstAmount = yshuliang * 0.9144;
						            commInfo.setFirstAmount(firstAmount);
					            } else if (firstUnit!= null && (firstUnit.getName().trim().equals(
									      "千克") || firstUnit.getName().trim().equals("公斤"))) {
					            	firstAmount = commAmount;
						            commInfo.setFirstAmount(commAmount);					            
								...
								if (seconedUnit != null && seconedUnit.getName().trim().equals("米")) {
									secondAmount = yshuliang * 0.9144;
						            commInfo.setSecondAmount(secondAmount);
								...
								commInfo.setFirstAmount(firstAmount);   //第一法定数量
								commInfo.setSecondAmount(secondAmount);//第二法定数量
								commInfo.setCommAmount(commAmount);    //数量		
								commInfo.setVersion(bcusexgbill.getVersion() == null?null:String.valueOf(bcusexgbill.getVersion()));//版本号
							}					
							commInfo.setGrossWeight(gossWeight);//毛重
							commInfo.setNetWeight(netWeight);//净重
							commInfo.setPieces(piece);
							switch (projectType){
							case ProjectType.BCUS:
								BaseCustomsDeclarationCommInfo info = bcusEncAction.saveCustomsinfoFromBill(new Request(CommonVars.getCurrUser()),commInfo,customsDeclaration,bcusexgbill,bcusimgbill);
	...
	private JButton getJButton1() {
		if (jButton1 == null) {
			...
			jButton1.addActionListener(new java.awt.event.ActionListener() { 
				public void actionPerformed(java.awt.event.ActionEvent e) {  
					setOk(false);
					DgFromMaterielEdit.this.dispose();
				}
			});
		}
		return jButton1;
	...
	private JButton getJButton5() {
		if (jButton5 == null) {
			...
			jButton5.addActionListener(new java.awt.event.ActionListener() { 
				public void actionPerformed(java.awt.event.ActionEvent e) { 
					if (JOptionPane.showConfirmDialog(DgFromMaterielEdit.this,
							"确定要删除吗？", "确认", 0) == 0) {						
