...
public class DgQueryQpSpecial extends JDialogBase {
	...
	private int impExpFlag = ImpExpFlag.SPECIAL;
	...
	private void initialize() {
		this.setSize(new Dimension(560, 356));
	...
	private JButton getBtnOk() {
		if(btnOk == null) {
			btnOk = new JButton();
			btnOk.setText("导入查询");
			btnOk.setBounds(new Rectangle(241, 44, 89, 26));
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					// List list=null;
					// list=baseEncAction.findAllLoadBGDFromQPXml(new
					// Request(CommonVars.getCurrUser()),
					// tfCustomsDeclarationCode.getText());
					...
					if(impExpFlag == ImpExpFlag.SPECIAL) {
						List lsResult = baseEncAction.findBGDFromQPXml(
								...
								ImpExpFlag.SPECIAL, condition, parameters);
						System.out
								.println("lsResult.size()=" + lsResult.size());
						initTable(lsResult);
					}
				}
			});
