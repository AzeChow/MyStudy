...
public class DgImpExpCommodityInfo extends JDialogBase {
	...
	public void showData(ImpExpCommodityInfo data) {
		...
		System.out.println(data.getPiece());
		...
		tfVersion.setText(data.getVersion() == null ? "" : data.getVersion()
				.toString());
		tfCmpVersion.setText(data.getCmpVersion()== null ? "" : data.getCmpVersion()
				.toString());
		...
		if(data.getTotalBoxNum()!=null&&!"".equals(data.getTotalBoxNum().trim())){
	...
	protected void fillData(ImpExpCommodityInfo data) {
		data.setBulks(tfBulks.getText() == null
						|| "".equals(tfBulks.getText()) ? null : Double
						.valueOf(tfBulks.getValue().toString()));
		data.setCountry((Country) this.cbbTradeCountry.getSelectedItem());
		data.setCurrency((Curr) this.cbbCurr.getSelectedItem());
		data.setGrossWeight(Double.valueOf(tfGrossWeight.getValue()
						.toString()));
		...
		data.setAmountPrice(CaleUtil.multiply(data.getQuantity() == null ? 0.0 : data
				.getQuantity(), data.getUnitPrice() == null ? 0.0 : data.getUnitPrice(), 5));
	...
	public void saveData() {
		...
		if (dataState == DataState.ADD) {
			...
			fillData(data);
			list.add(data);
			this.encAction.saveImpExpRequestBill(new Request(CommonVars
					.getCurrUser(), true), data.getImpExpRequestBill());
			this.encAction.saveImpExpCommodityInfo(new Request(CommonVars
					.getCurrUser()), list);
			...
			this.encAction.saveImpExpRequestBill(new Request(CommonVars
					.getCurrUser(), true), impExpCommodityInfo
	...
	private Double getAmountStr() {
		tfQuantity.validate();
		try {
			BigDecimal amount = new BigDecimal(tfQuantity.getValue() == null ? "0.0"
					: tfQuantity.getValue().toString());
			BigDecimal bd = amount.multiply(new BigDecimal(tfUnitPrice.getValue() == null ? "0.0"
			...
			return bd.setScale(amountNum,BigDecimal.ROUND_HALF_UP).doubleValue();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return 0.0;
			...
			if (tfinvGrossWeight.getText() != null
				...
				amount = Double
						.parseDouble(tfQuantity.getValue() == null ? "0.0"
								: tfQuantity.getValue().toString())
						* Double
								.parseDouble(tfinvGrossWeight.getValue() == null ? "0.0"
										: tfinvGrossWeight.getValue()
												.toString());
			} else {
				amount = Double
			...
			if (tfinvNetWeight.getText() != null
				...
				amount = Double
						.parseDouble(tfQuantity.getValue() == null ? "0.0"
								: tfQuantity.getValue().toString())
						* Double
								.parseDouble(tfinvNetWeight.getValue() == null ? "0.0"
										: tfinvNetWeight.getValue().toString());
			} else if("千克".equals(tfUnit.getText())){
				amount=Double.parseDouble(tfQuantity.getValue() == null ? "0.0"
						: tfQuantity.getValue().toString());
			} else {
	...
	private void initUIComponents() {
	    ...
	    //数量,单价,总金额
	    if(other!=null){
	    	CustomFormattedTextFieldUtils.setFormatterFactory(this.tfQuantity, other.getCustomAmountNumSq());
	    	CustomFormattedTextFieldUtils.setFormatterFactory(this.tfUnitPrice, other.getCustomPriceNumSq());
	    	CustomFormattedTextFieldUtils.setFormatterFactory(this.tfAmount, other.getCustomTotalNumSq());
	    }else{
	    	CustomFormattedTextFieldUtils.setFormatterFactory(this.tfQuantity, 9);	    
	    	CustomFormattedTextFieldUtils.setFormatterFactory(this.tfUnitPrice, 9);
...
//			tfStartBoxNo.setText("0");
			...
			tfStartBoxNo.addCaretListener(new javax.swing.event.CaretListener() {
	...
	private void setBoxNo() {
		if(CommonUtils.isEmpty(tfStartBoxNo.getText())&&!CommonUtils.isEmpty(tfEndBoxNo.getText())) {
			tfBoxNo.setText(tfEndBoxNo.getText());
		} else if(CommonUtils.isEmpty(tfEndBoxNo.getText())&&!CommonUtils.isEmpty(tfStartBoxNo.getText())) {
	...
	/**
	 * 出口申请单手工录入时，输入起始箱号和截止箱号，不能自动输出箱数，要求实现：
	 *  1、当申请单的起始箱号和截止箱号是纯数字时，启用公式 箱数= 截止箱号-起始箱号+1 
	 *  2、当申请单的起始箱号和截止箱号是两位字母+6为数字的格式时，
	 *  使用公式 箱数=截止箱号后六位数字-起始箱号后六位数字+1
	 */
	private void setTotalBoxNum() {
		...
		if (!"".equals(tfStartBoxNo.getText())
			...
			if(!tfStartBoxNo.getText().trim().equals("0")){
				 startBoxNo = Integer.valueOf(tfStartBoxNo.getText().trim().replaceFirst("^(0+)", ""));
			}else{
				 startBoxNo = Integer.valueOf(tfStartBoxNo.getText().trim());
			}
			if(!tfEndBoxNo.getText().trim().equals("0")){
				 endBoxNo = Integer.valueOf(tfEndBoxNo.getText().trim().replaceFirst("^(0+)", ""));
			}else{
			...
			tfTotalBoxNum.setText(String.valueOf(endBoxNo-startBoxNo+1));
				...
				&& !"".equals(tfEndBoxNo.getText())
				&& RegexUtil.checkMatch(tfStartBoxNo.getText().trim(), "^[^0-9]{2}[0-9]{6}$")
				&& RegexUtil.checkMatch(tfEndBoxNo.getText().trim(), "^[^0-9]{2}[0-9]{6}$")){
			String start = tfStartBoxNo.getText().trim().substring(2, tfStartBoxNo.getText().trim().length());
			String end = tfEndBoxNo.getText().trim().substring(2, tfEndBoxNo.getText().trim().length());
	...
	private JTextField getTfTotalBoxNum() {
		if (tfTotalBoxNum == null) {
			...
			tfTotalBoxNum.addCaretListener(new javax.swing.event.CaretListener() {
				public void caretUpdate(javax.swing.event.CaretEvent e) {
