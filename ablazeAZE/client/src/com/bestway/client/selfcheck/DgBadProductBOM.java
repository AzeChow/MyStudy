...
public class DgBadProductBOM extends JDialogBase {
	...
	private JCalendarComboBox getCbEndDate() {
		if (cbEndDate == null) {
			...
			cbEndDate.setLocation(new Point(401, 156));
	...
	private JPanel getJPanel1() {
		if (jPanel1 == null) {
			...
			lbEndDate.setBounds(new Rectangle(339, 157, 53, 18));
			...
			lbHsSpec = new JLabel();
			lbHsSpec.setBounds(new Rectangle(342, 55, 77, 18));
			...
			lbPtSpec = new JLabel();
			lbPtSpec.setBounds(new Rectangle(342, 78, 77, 19));
			...
			lbHsNoEnd
					.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			lbHsNoEnd.setLocation(new Point(335, 130));
			...
			lbPtNoEnd.setLocation(new Point(334, 104));
	...
	private JCheckBox getCbFactoryName() {
		if (cbFactoryName == null) {
			...
			cbFactoryName.setLocation(new Point(312, 160));
	...
	public void queryData(){
		...
		if (!tfHsName.getText().trim().equals("")) {
			System.out.println("把报关商品SET进实体");
	...
	class Find extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {//后台计算
			...
			List list = null;
			//查询
			CommonProgress.showProgressDialog(DgBadProductBOM.this);
...
//			}
//			else{
			list = casCheckAction.getCurrentBadProductBOM(new Request(CommonVars.getCurrUser()),casConvert,billDetail);
//			}
			return list;
		}
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					List<TempObject> list = CasQuery.getInstance()
							.getFactoryAndFactualCustomsRalation(false,
									MaterielType.FINISHED_PRODUCT, new ArrayList());
	...
	private JButton getBtnComplexCode() {
		if (btnComplexCode == null) {
			...
			btnComplexCode.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					String materialType = getMaterielType();
					Object object = CasQuery.getInstance().findHsNoFromStatCusNameRelationHsnmByXiaoK(MaterielType.FINISHED_PRODUCT);
					if (object != null) {
	...
	private JButton getBtnProductNo() {
		if (btnProductNo == null) {
			...
			.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Object object = CasQuery.getInstance().findBWorkOrder();
					if (object != null) {
						tfProductNo
								.setText((String) ((TempObject) object)
										.getObject());
