...
public class DgSupplierBalanceAllInfo extends JDialogBase {
	 ...
	 * 
	 */
	public DgSupplierBalanceAllInfo() {
		super();
		...
		tableModel = new AttributiveCellTableModel((MultiSpanCellTable) jTable,
				list, new JTableListModelAdapter() {
					public List<JTableListColumn> InitColumns() {
						...
						list.add(addColumn("业务类型", "type", 60));// 5
						list.add(addColumn("期初", "amountFirst", 60));// 6
						...
						list.add(addColumn("关封余量", "remain", 60));// 22
						return list;
					}
				});
	...
	public void combineTable() {
		((MultiSpanCellTable) jTable).splitRows2(new int[] { 1, 2, 3, 4, 19,
				21, 22 });
		((MultiSpanCellTable) jTable).combineRowsByIndeies(new int[] { 1, 2, 3,
				4, 19, 21, 22 }, new int[] { 1, 2, 3, 4, 19, 21, 22 });
	}
	...
	class Find extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {// 后台计算
			...
			// 查询
			CommonProgress.showProgressDialog(DgSupplierBalanceAllInfo.this);
			CommonProgress.setMessage("系统正在执行查询，请稍后...");
			// 查询返回结果
			return casCheckAction.getCurrentBalanceInfo(new Request(CommonVars
		...
		protected void done() {// 更新视图
			...
			try {
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(DgSupplierBalanceAllInfo.this,
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Object obj = CasQuery.getInstance()
							.findHsNameFromStatCusNameRelationHsn(
									isM == true ? MaterielType.MATERIEL
	...
	private JButton getBtnRelation() {
		if (btnRelation == null) {
			...
			btnRelation.setBounds(new Rectangle(600, 43, 95, 24));
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (tableModel.getCurrentRow() == null) {
						JOptionPane.showMessageDialog(
								DgSupplierBalanceAllInfo.this, "请选择你要查看的资料",
								"确认", 1);
						return;
					}
