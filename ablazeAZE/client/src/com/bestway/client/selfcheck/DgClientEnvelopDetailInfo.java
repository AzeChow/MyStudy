...
public class DgClientEnvelopDetailInfo extends JDialogBase {
	 ...
	 * 
	 */
	public DgClientEnvelopDetailInfo() {
		super();
	...
	private void initialize() {
        this.setSize(new Dimension(839, 609));
        this.setContentPane(getJContentPane());
        this.setTitle("(客户)关封明细表");
	...
	public List getManufacturer() {
		List list = this.materialManageAction.findScmCocscustomerByXiaok(new Request(
	...
	private JButton getBtnQuery() {
		if (btnQuery == null) {
			...
			btnQuery.setText("查询");
			btnQuery.setBounds(new Rectangle(623, 23, 87, 31));
	...
	class Find extends SwingWorker{
		...
		protected Object doInBackground() throws Exception {//后台计算
			//查询返回结果
			List<CustomsEnvelopCommodityInfo> list = null;
			//查询
			CommonProgress.showProgressDialog(DgClientEnvelopDetailInfo.this);
		...
		//	list = casCheckAction.getCurrentBillDetailBom(new Request(CommonVars.getCurrUser()),casConvert,billDetail);
			...
			list = transferFactoryManageAction.getEnvelopBillDetailout(new Request(CommonVars.getCurrUser()),conditions,conditions1);
			return list;
	...
	private JButton getBtnPrint() {
		if (btnPrint == null) {
			...
			btnPrint.setText("打印");
			btnPrint.setBounds(new Rectangle(718, 23, 87, 31));
	...
	private JButton getBtnClose() {
		if (btnClose == null) {
			...
			btnClose.setText("关闭");
			btnClose.setBounds(new Rectangle(718, 65, 87, 31));
	...
	public void setQueryCondition(Date startDate,Date endDate,String hsCode,
		...
		this.tfWareName.setText(hsName);//报关名称
		this.tfWareSpec.setText(hsSpec);//报关规格
		this.cbbClientName.setSelectedItem(scmCoc);//客户
	...
	private JButton getBtnRelation() {
		if (btnRelation == null) {
			...
			btnRelation.setBounds(new Rectangle(623, 65, 87, 31));
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (tableModel.getCurrentRow() == null) {
						JOptionPane.showMessageDialog(
								DgClientEnvelopDetailInfo.this,
								"请选择你要查看的资料", "确认", 1);
						return;
					}
