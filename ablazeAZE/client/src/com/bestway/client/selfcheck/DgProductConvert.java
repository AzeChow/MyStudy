...
public class DgProductConvert extends JDialogBase {
	...
	private JPanel getJPanel1() {
		if (jPanel1 == null) {
			...
			lbProductNo.setBounds(new Rectangle(27, 55, 77, 18));
			...
			lbHsSpec = new JLabel();
			lbHsSpec.setBounds(new Rectangle(343, 54, 77, 18));
			lbHsSpec.setText("成品报关名称");
			
			lbComplexCode = new JLabel();
			lbComplexCode.setBounds(new Rectangle(26, 79, 83, 18));
			lbComplexCode.setText("成品报关编号");
			lbPtSpec = new JLabel();
			lbPtSpec.setBounds(new Rectangle(344, 80, 76, 19));
			lbPtSpec.setText("成品工厂名称");
			jLabel11 = new JLabel();
			jLabel11.setText("至");
			...
			lbHsNoEnd
					.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			lbHsNoEnd.setLocation(new Point(338, 129));
			...
			lbPtNoEnd.setLocation(new Point(337, 105));
			lbPtPartEnd = new JLabel();
			lbPtPartEnd.setText("成品工厂料号");
			lbPtPartEnd.setSize(new Dimension(80, 20));
			lbPtPartEnd.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			lbPtPartEnd.setLocation(new Point(335, 30));
			...
			lbHsNoBegin.setLocation(new Point(20, 132));
			...
			lbPtNoBegin.setLocation(new Point(20, 105));
			lbPtPartBegin = new JLabel();
			lbPtPartBegin.setText("成品工厂料号");
			lbPtPartBegin.setSize(new Dimension(80, 20));
			lbPtPartBegin.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			lbPtPartBegin.setLocation(new Point(20, 31));
			jPanel1 = new JPanel();
	...
	private JButton getBtQuerry() {
		if (btQuerry == null) {
			...
			btQuerry.setText("查询");
			btQuerry.setSize(new Dimension(98, 25));
...
//					"<=", CommonVars.getBeginDate(date), null));
		...
		new Find(conditions,"",billDetail,PtNoBegin,casConvert).execute();
	...
	class Find extends SwingWorker{
		...
		public Find(List<Condition> conditions, String orderBy,
				String billDetail,String PtNoBegin,CasConvert casConvert) {
		...
		protected Object doInBackground() throws Exception {//后台计算
			...
			List list = null;
			//查询
			CommonProgress.showProgressDialog(DgProductConvert.this);
...
//			}
	...
	private JButton getBtprint() {
		if (btprint == null) {
			...
			btprint.setSize(new Dimension(99, 25));
	...
	private JButton getBtClose() {
		if (btClose == null) {
			btClose = new JButton();
			btClose.setText("关闭");
			btClose.setSize(new Dimension(97, 25));
			btClose.setLocation(new Point(627, 143));
			btClose.addActionListener(new java.awt.event.ActionListener() {

				public void actionPerformed(java.awt.event.ActionEvent e) {

					DgProductConvert.this.dispose();
		...
		tableModel = new AttributiveCellTableModel((MultiSpanCellTable)jTable, list,
				new JTableListModelAdapter() {
					public List<JTableListColumn> InitColumns() {
						...
						list.add(addColumn("单据日期", "validDate", 100)); //1
						list.add(addColumn("单据类型", "billTypeName", 100)); //2
						list.add(addColumn("单据号码", "billNo", 100)); //3
						list.add(addColumn("工厂商品编码", "ptPart", 100)); //4
						list.add(addColumn("工厂商品名称", "ptName", 100)); //5
						list.add(addColumn("工厂商品规格", "ptSpec", 100)); //6
						list.add(addColumn("工厂商品单位", "ptUnitName", 100)); //7
						list.add(addColumn("工厂商品数量", "ptAmount", 100)); //8
						list.add(addColumn("报关商品编码", "complexCode", 100)); //9
						list.add(addColumn("报关商品名称", "hsName", 100)); //10
						list.add(addColumn("报关商品规格", "hsSpec", 100)); //11
						list.add(addColumn("报关商品单位", "hsUnitName", 100)); //12
						list.add(addColumn("报关商品数量", "hsAmount", 100)); //13
						list.add(addColumn("工厂料件编码", "ptNo", 100)); //14
						list.add(addColumn("工厂料件名称", "factoryName", 100)); //15
						list.add(addColumn("工厂料件规格", "factorySpec", 100)); //16
						list.add(addColumn("工厂料件单位", "calUnitName", 100)); //17
						list.add(addColumn("报关料件编码", "mcomplexCode", 100)); //18
						list.add(addColumn("报关料件名称", "mptName", 100)); //19
						list.add(addColumn("报关料件规格", "mptDeSpec", 100)); //20
						list.add(addColumn("报关料件单位", "mptUnitName", 100)); //21
						list.add(addColumn("单耗", "unitWaste", 100)); //22
						list.add(addColumn("损耗", "waste", 100)); //23
						list.add(addColumn("单项用量", "unitUsed", 100)); //24
						list.add(addColumn("总用量", "totalDosage", 100)); //25
						list.add(addColumn("折算系数", "unitConvert", 100)); //26
						list.add(addColumn("报关数量", "ptTotal", 100)); //27
						list.add(addColumn("制单号", "productNo", 100)); //28
						return list;
					}
				});
	...
	private JButton getBtnComplexCode() {
		if (btnComplexCode == null) {
			...
			btnComplexCode.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					String materialType = getMaterielType();
					Object object = CasQuery.getInstance().findHsNameFromStatCusNameRelationHsn(materialType);
					if (object != null) {
						tfComplexCode.setText((String) ((TempObject) object)
								.getObject3());
					}
				}
			});
	...
	private JCalendarComboBox getCbEndDate() {
		if (cbEndDate == null) {
			...
			cbEndDate.setLocation(new Point(413, 156));
	...
	private JButton getBtprint2() {
		if (btprint2 == null) {
			...
			btprint2.setBounds(new Rectangle(626, 105, 98, 25));
	...
	private JTextArea getJTextArea() {
		if (jTextArea == null) {
			...
			jTextArea.setBounds(new Rectangle(179, 153, 0, 0));
		}
		return jTextArea;
	}
