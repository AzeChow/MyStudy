 ...
 */
public class DgDzscExgSort extends JDialogBase {
	...
	private void move(boolean isup) {
		if (jList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(DgDzscExgSort.this, "请选择要移动的项!",
					"提示", 2);
			return;
		}
		...
		DzscEmsExgBill selectinfo = null;
		DzscEmsExgBill preinfo = null;
		if (isup) {// 向上移动
			selectinfo = (DzscEmsExgBill) selectObj;
			selectinfo.setSeqNum(selectinfo.getSeqNum() - 1);
			preinfo = (DzscEmsExgBill) jList.getModel().getElementAt(x - 1);
	...
	private JButton getBtnOk() {
		if (btnOk == null) {
			...
			btnOk.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Vector list = DgDzscExgSort.this.getList();
					dzscAction.saveDzscEmsExgBill(new Request(CommonVars
							.getCurrUser()), list);
					isOk = true;
					dispose();
				}
