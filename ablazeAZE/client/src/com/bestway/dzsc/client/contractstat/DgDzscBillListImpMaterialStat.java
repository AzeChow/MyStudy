...
public class DgDzscBillListImpMaterialStat extends JDialogBase {
	 ...
	 * 
	 */
	public DgDzscBillListImpMaterialStat() {
		super();
		initialize();
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (tableModel.getList().size() < 1) {
						JOptionPane.showMessageDialog(
								DgDzscBillListImpMaterialStat.this, "没有数据可以打印",
					...
					InputStream reportStream = DgDzscBillListImpMaterialStat.class
							.getResourceAsStream("report/DzscBillListImpMaterialStat.jasper");
	...
	private JButton getBtnQuery() {
		if (btnQuery == null) {
			...
			btnQuery.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					List<DzscBillListImpMaterialStat> list = contractStatAction
							.findBillListImpMaterialStat(new Request(CommonVars
									.getCurrUser()), contract, cbbBeginDate
				...
				new JTableListModelAdapter() {
					public List InitColumns() {
						...
						list.add(addColumn("转厂进口数", "transferFactoryImport",
								100));
						list.add(addColumn("料件复出数", "backMaterialReturn", 100));
						list.add(addColumn("料件退换进口数", "exchangeImport", 100));
						list.add(addColumn("料件退换出口数", "exchangeExport", 100));
