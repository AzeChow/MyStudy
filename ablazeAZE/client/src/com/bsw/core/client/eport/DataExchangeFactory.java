...
public class DataExchangeFactory {
	...
	private void init() {
		...
		try {
			...
			Logger.getLogger(DataExchangeFactory.class.getName()).log(
					Level.SEVERE, "加载QP配置文件：" + configPath, ex);
		} finally {
			if (in != null) {
				try {
					in.close();
	...
	private <T> T getProxyObject(String moduleId, Class<T> interfClass) {
		...
		try {
			...
			T proxy = (T) Proxy.newProxyInstance(classLoader,
					new Class[] { interfClass }, new MethodInvocation(target));
			System.out.println("proxy instance : " + proxy);
			return proxy;
		} catch (Exception ex) {

			throw new RuntimeException("加载" + moduleId + "数据交换接口失败:"
					+ className, ex);
		}
	}
	...
	private URLClassLoader getClassLoader() {
		if (classLoader == null) {
			...
			try {
				...
				classLoader = new CustomURLClassLoader(getJarURLs(jarUrls),
						Thread.currentThread().getContextClassLoader());
			} catch (Exception ex) {
				Logger.getLogger(DataExchangeFactory.class.getName()).log(
						Level.SEVERE, "无法加载QP 数据交换接口jar:" + jarUrls, ex);
				throw new RuntimeException(ex);
			}
		}
		return classLoader;
