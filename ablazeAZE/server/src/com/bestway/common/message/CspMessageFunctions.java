 ...
 */
package com.bestway.common.message;
 ...
 * Preferences - Java - Code Style - Code Templates
 */
public class CspMessageFunctions {
	/**
	...
	public static String formatVariant(Object value, String valueFmt)
		...
		if (mFmt.charAt(0) == 'R') {
			...
			if (mFmt.charAt(0) == 'X') {
				...
				// if(value.toString().getBytes().length>=mFmt.length()){
...
//			temp = new String(bytes, 0, bytesCount - 1);
//		} else {
//			temp = new String(bytes, 0, bytesCount);
//		}
//		String mStr = "";
//		for (int i = 0; i < 390; i++) {// 255
//			mStr = mStr + " ";
//		}
//		return new String((temp + mStr).getBytes(), 0, bytesCount);
//	}
     ...
     */
    public static String cut(String s, int length) throws Exception {
...
//        return new String(bytes, 0, i, "Unicode");
        ...
        for (int i = 1; i < ss.length; i++) {
            count += (ss[i].getBytes("GBK").length > 1 ? 2 : 1);
            if (count > length) {
                break;
            }
            sb.append(ss[i]);
	 ...
	 * @return
	 */
	public static boolean copyFile(File file, String destDir) {
		try {
			File destFile = new File(destDir + File.separator + file.getName());
			if (destFile.exists()) {
				destFile.delete();
	...
	public static void checkSendDirIsAvaliable(String sendDir) {
		if (sendDir == null || "".equals(sendDir.trim())) {
			throw new RuntimeException("报文的发送路径设置不能为空");
		}
		File file = new File(sendDir);
		if (!file.exists()) {
			throw new RuntimeException("报文的发送路径设置不正确");
		}
	}

	public static void checkRecvDirIsAvaliable(String recvDir) {
		if (recvDir == null || "".equals(recvDir.trim())) {
			throw new RuntimeException("回执的接收路径设置不能为空");
		}
		File file = new File(recvDir);
		if (!file.exists()) {
			throw new RuntimeException("回执的接收路径设置不正确");
		}
	}

	public static void checkFinallyDirIsAvaliable(String finallyDir) {
		if (finallyDir == null || "".equals(finallyDir.trim())) {
			throw new RuntimeException("回执的处理完成存放路径设置不能为空");
		}
		File file = new File(finallyDir);
		if (!file.exists()) {
			throw new RuntimeException("回执的处理完成存放路径设置不正确");
		}
	}
}
