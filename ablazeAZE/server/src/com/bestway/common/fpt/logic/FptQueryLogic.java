...
import com.bestway.common.fpt.entity.TempEnvelopComplex;
import com.bestway.common.fpt.entity.TransCustomsCollate;
...
public class FptQueryLogic {
	...
	public List findCustomsEnvelopComplex(boolean isImport, boolean isSeqNum) {
		...
		List list = this.fptManageDao.findCustomsEnvelopComplex(
				isImport, isSeqNum);
	...
	public List findCustomsEnvelopList(String envelopCodeValue, Integer seqNum,
		...
		// beginDate, endDate);
		// for (int i = 0; i < list.size(); i++) {
		// CustomsEnvelopCommodityInfo commInfo = (CustomsEnvelopCommodityInfo)
		// list
		// .get(i);
	...
	public List findPutRecord() {
		...
		// temp.setBillSum5(fd(temp.getBillSum1()) + fd(temp.getBillSum4()));//
		// 备案差异
		// }
	...
	public List findTransferClienCollate(Integer projectType, String emsNo,
		...
		for (int i = 0; i < j; i++) {
			...
			if (hsc.get(key) == null) {
				...
				temp.setBill1("-- 期初");
				Double begNum = 0.0;
				Double begInitNum = fptManageDao
			...
			if (hs.get(key) == null) {
				...
				Double transNum = this.fptManageDao
						.findCustomsDeclarationAmountByEnvelopNoAndSeqNum(
	...
	public List traCustomsCollate(String year, boolean isImpType) {
		...
		List distinctLs = this.fptManageDao.findDistinctTrans(year,
	...
	private void getTransNum(String year, Boolean isImpExpType) {
		transHs.clear();
		List ls = fptManageDao.getTransNum(year, isImpExpType);
	...
	private void getTransMoney(String year, Boolean isImpExpType) {
		transMoneyHs.clear();
		List ls = fptManageDao.getTransMoney(year, isImpExpType);
		for (int i = 0; i < ls.size(); i++) {
			Object[] obj = (Object[]) ls.get(i);
			Integer seqNum = (Integer) obj[0];// 序号
			ScmCoc scm = (ScmCoc) obj[1];
	...
	private void getCustomsNum(String year, Boolean isImpExpType) {
		customsHs.clear();
		List ls = fptManageDao.getCustomsNum(year, isImpExpType);
		for (int i = 0; i < ls.size(); i++) {
			Object[] obj = (Object[]) ls.get(i);
			Integer seqNum = (Integer) obj[0];// 序号
			ScmCoc scm = (ScmCoc) obj[1];
	...
	private void getCustomsMoney(String year, Boolean isImpExpType) {
		...
		List ls = fptManageDao.getCustomsMoney(year, isImpExpType);
		for (int i = 0; i < ls.size(); i++) {
			Object[] obj = (Object[]) ls.get(i);
			Integer seqNum = (Integer) obj[0];// 序号
			ScmCoc scm = (ScmCoc) obj[1];
	...
	private void getExistEmsExg() {
		hsemsinexg.clear();
		List ls = fptManageDao.findEmsHeadH2kExg();
		for (int i = 0; i < ls.size(); i++) {
	...
	private void getExistEmsImg() {
		hsemsinimg.clear();
		List ls = fptManageDao.findEmsHeadH2kImg();
		for (int i = 0; i < ls.size(); i++) {
	 ...
	 * @return
	 */
	public List findMakeFptBillFromCasBillInfo(String emsNo, String casBillNo,
			String ptNo, String fptCopNo, String complexCode,
			Date beginCasDate, Date endCasDate, Date beginFptDate,
			Date endFptDate) {
		List lsResult = new ArrayList();
		List list = this.fptManageDao
				.findMakeFptBillFromCasBillInfo(true, emsNo, casBillNo, ptNo,
						fptCopNo, complexCode, beginCasDate, endCasDate,
						beginFptDate, endFptDate);
		for (int i = 0; i < list.size(); i++) {
			Object[] objs = (Object[]) list.get(i);
			if (objs[0] != null) {
				MakeFptBillFromCasBill makeInfo = (MakeFptBillFromCasBill) objs[0];
				makeInfo.setTempFptBillItem((FptBillItem) objs[1]);
				makeInfo.setTempBillDetail((BillDetail) objs[2]);
				lsResult.add(makeInfo);
			}
		}
		list = this.fptManageDao.findMakeFptBillFromCasBillInfo(
				false, emsNo, casBillNo, ptNo, fptCopNo, complexCode,
				beginCasDate, endCasDate, beginFptDate, endFptDate);
		for (int i = 0; i < list.size(); i++) {
			Object[] objs = (Object[]) list.get(i);
			if (objs[0] != null) {
				MakeFptBillFromCasBill makeInfo = (MakeFptBillFromCasBill) objs[0];
