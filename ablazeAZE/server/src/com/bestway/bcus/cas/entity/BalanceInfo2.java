package com.bestway.bcus.cas.entity;

import com.bestway.common.BaseScmEntity;
import com.bestway.common.CommonUtils;
/**
 * 平衡信息表
 */
public class BalanceInfo2 extends BaseScmEntity {
    private static final long serialVersionUID = CommonUtils
                                                       .getSerialVersionUID();
    /**
	 * 名称
	 */
    private String            name             = "";                         
    /**
	 * 规格
	 */
    private String            spec             = "";                          
    /**
	 * 单位
	 */
    private String            unitName         = "";
	
    /**
	 * 库存原料
	 */
    private Double            f1               = 0.0;                         
    /**
	 * 半成品、成品折原料
	 */
    private Double            f2               = 0.0;                         
    /**
	 * 库存原材料合计
	 */
    private Double            f3               = 0.0;  
    /**
	 * 库存原料
	 */
    private Double            f1A               = 0.0;                         
    /**
	 * 半成品折原料
	 */
    private Double            f2A               = 0.0;                         
    /**
	 * 成品折原料
	 */
    private Double            f3A               = 0.0;   
    /**
	 * 库存原材料合计
	 */
    private Double            f4               = 0.0;                         
    /**
	 * 已收货未转厂数
	 */
    private Double            f5               = 0.0;
    /**
	 * (内购)实际进料
	 */
    private Double            f5A               = 0.0;   
    /**
	 * (内购)已结案合同发票核销数
	 */
    private Double            f5B               = 0.0;   
    /**
	 * 已转厂未收货数
	 */
    private Double            f6               = 0.0;                         
    /**
	 * 已送货未转厂数
	 */
    private Double            f7               = 0.0;                         
    /**
	 * 已转厂未送货数
	 */
    private Double            f8               = 0.0;                         
    /**
	 * (内购)成品出货内购耗料
	 */
    private Double            f9               = 0.0;                         
    /**
	 * 合同库存数(监管) = f5+f6+f7-f8-f9
	 */
    private Double            f10              = 0.0;                         
    /**
	 * 合同进口数
	 */
    private Double            f11              = 0.0;                         
    /**
	 * 合同出口耗料
	 */
    private Double            f12              = 0.0;                         
    /**
	 * 合同结余数  f11-f12
	 */
    private Double            f13              = 0.0;                         
    /**
	 * 短溢情况  f10-f13
	 */
    private Double            f14              = 0.0;   
    
    
    
    public Double getF1() {
		return f1;
	}




	public void setF1(Double f1) {
		this.f1 = f1;
	}




	public Double getF10() {
		return f10;
	}




	public void setF10(Double f10) {
		this.f10 = f10;
	}




	public Double getF11() {
		return f11;
	}




	public void setF11(Double f11) {
		this.f11 = f11;
	}




	public Double getF12() {
		return f12;
	}




	public void setF12(Double f12) {
		this.f12 = f12;
	}




	public Double getF13() {
		return f13;
	}




	public void setF13(Double f13) {
		this.f13 = f13;
	}




	public Double getF14() {
		return f14;
	}




	public void setF14(Double f14) {
		this.f14 = f14;
	}




	public Double getF1A() {
		return f1A;
	}




	public void setF1A(Double f1a) {
		f1A = f1a;
	}




	public Double getF2() {
		return f2;
	}




	public void setF2(Double f2) {
		this.f2 = f2;
	}




	public Double getF2A() {
		return f2A;
	}




	public void setF2A(Double f2a) {
		f2A = f2a;
	}




	public Double getF3() {
		return f3;
	}




	public void setF3(Double f3) {
		this.f3 = f3;
	}




	public Double getF3A() {
		return f3A;
	}




	public void setF3A(Double f3a) {
		f3A = f3a;
	}




	public Double getF4() {
		return f4;
	}




	public void setF4(Double f4) {
		this.f4 = f4;
	}




	public Double getF5() {
		return f5;
	}




	public void setF5(Double f5) {
		this.f5 = f5;
	}




	public Double getF5A() {
		return f5A;
	}




	public void setF5A(Double f5a) {
		f5A = f5a;
	}




	public Double getF5B() {
		return f5B;
	}




	public void setF5B(Double f5b) {
		f5B = f5b;
	}




	public Double getF6() {
		return f6;
	}




	public void setF6(Double f6) {
		this.f6 = f6;
	}




	public Double getF7() {
		return f7;
	}




	public void setF7(Double f7) {
		this.f7 = f7;
	}




	public Double getF8() {
		return f8;
	}




	public void setF8(Double f8) {
		this.f8 = f8;
	}




	public Double getF9() {
		return f9;
	}




	public void setF9(Double f9) {
		this.f9 = f9;
	}




	/**
     * 取得唯一键值
     * @return name+ptSpec+ptUnitName 名称+规格+单位.
     */
    public String getKey(){
        String ptSpec  = (this.spec == null || "".equals(this.spec))?"":"/"+this.spec;   
        String ptUnitName = (this.unitName == null || "".equals(this.unitName))?"":"/"+this.unitName; 
        return this.name+ptSpec+ptUnitName;
    }
    
    


    /**
     * 取得商品名称
     * @return name 商品名称.
     */
    public String getName() {
        return name;
    }



    /**
     * 设置商品名称
     * @param name  商品名称.
     */
    public void setName(String name) {
        this.name = name;
    }



    /**
     * 取得商品规格
     * @return spec 商品规格.
     */
    public String getSpec() {
        return spec;
    }



    /**
     * 设置商品规格
     * @param spec  商品规格.
     */
    public void setSpec(String spec) {
        this.spec = spec;
    }



    /**
     * 取得单位名称
     * @return unitName 单位名称.
     */
    public String getUnitName() {
        return unitName;
    }



    /**
     * 设置单位名称
     * @param unitName  单位名称.
     */
    public void setUnitName(String unitName) {
        this.unitName = unitName;
    }

}
