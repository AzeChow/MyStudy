...
public class ReloadableSizedIconSource extends DefaultIconSource implements SizedIconSource {
    ...
    public void reload(IconSize size) {
        Assert.notNull(size);
        this.iconSize = size;
        DefaultImageSource images=(DefaultImageSource) this.getImages();
        
        Iterator keys = images.getImageResources().keySet().iterator();
        if (!keys.hasNext()) {
    ...
    private void reloadIconImage(String key) {
        ...
        ImageIcon icon = (ImageIcon)this.getIcon(key);
        if (icon != null) {
            Image image = getImages().getImage(appendIconSizeSuffix(key));
            icon.setImage(image);
        }
    }
