...
public class DgSelectBrokerCorp extends JDialogBase {
	...
	private List<BrokerCorp> existBrokerCorpList = null;
	protected boolean isMulti = false;
	public boolean isOk = false;
	private Object returnValue;
	
	public DgSelectBrokerCorp() {
     ...
     * 初始化代理公司选择
     *
     * @param existBrokerCorpList -过滤已存在记录
     * @param _isMulti 是否多选
     */
    public DgSelectBrokerCorp(List<BrokerCorp> existBrokerCorpList, boolean _isMulti) {
        super();
        isMulti = _isMulti;
        this.existBrokerCorpList = existBrokerCorpList;
        this.setTitle("请选择代理报关公司");
        this.setBounds(new Rectangle(800,500));
		pisAction = (PisAction) CommonVars.getApplicationContext()
				.getBean("pisAction");
    ...
    public List<BrokerCorp> getSelectValue() {
        if (getReturnValue() != null) {
            return (List<BrokerCorp>) getReturnValue();
        }
			...
			btnOk.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (isMulti) {
						...
						if (res.isEmpty()) {
							JOptionPane.showMessageDialog(DgSelectBrokerCorp.this, "必须选择一条记录！", "提示",
						...
						if (res.size() > 1) {
							JOptionPane.showMessageDialog(DgSelectBrokerCorp.this, "只能选择一条记录！", "提示",
									JOptionPane.INFORMATION_MESSAGE);
							return;
						}
	...
	private List getDataSource(){
		System.out.println(pisAction);
		List listBrokerCorp =  pisAction.findBrokerCorp(new Request(CommonVars.getCurrUser()));

        // 去除已选选项
        if (existBrokerCorpList != null) {
            listBrokerCorp.removeAll(existBrokerCorpList);
        }

        return listBrokerCorp;
	}
